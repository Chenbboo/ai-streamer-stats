...
当前发布包包含 V010–V107；已核实完成 V105 的环境从 V106 执行。V106 新增立项人员跨项目投入计划字段；V107 清理公司老板的个人菜单权限快照，改由当前角色菜单管理。部署前须备份数据库，恢复旧菜单快照需使用备份中的对应记录。先依次执行迁移，再启动新版后端；发布门禁检查新字段和老板快照清理状态。
