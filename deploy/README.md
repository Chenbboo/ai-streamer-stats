...
当前发布包包含 V010–V119。V116 为项目进度汇报增加文字成果凭证字段；主项目进度由负责人独立填报，不再根据子项目自动计算。V117 新增珠宝成品退供预警邮件的每日发送记录表。V118 允许样品与非样品商品共用 SKU，库存等记录仍按商品 ID 分开。V119 扩大 KPI 实际值精度，支持以万元录入并按元核算。先执行迁移，再启动新版后端。迁移可重复执行。
