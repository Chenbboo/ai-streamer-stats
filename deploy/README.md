...
发布包包含 V010–V102。默认从 V010 执行；已核实完成 V096 的环境，本次依次执行 V097 至 V102。V097–V098 新增公司公共费用及长项目名兼容，V099 扩展立项预期利润率范围，V100 将公共费用改为按日计入，V101 新增公司税率和项目税后利润快照，V102 新增珠宝采购单约定退货日期（历史记录留空）。所有迁移须在发布新版后端前执行，发布门禁会校验双方对应表、字段、菜单和成本类别，并保留草稿及创建防重校验：
