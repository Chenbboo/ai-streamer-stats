...
if [[ ! "$first_migration" =~ ^[1-9][0-9]*$ ]] || (( first_migration < 10 || first_migration > 96 )); then
  echo "first migration version must be between 10 and 96" >&2
  exit 1
fi
