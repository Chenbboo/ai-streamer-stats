...
if [[ ! "$first_migration" =~ ^[1-9][0-9]*$ ]] || (( first_migration < 10 || first_migration > 101 )); then
  echo "first migration version must be between 10 and 101" >&2
  exit 1
fi
...
for version in $(seq -w 10 101); do
