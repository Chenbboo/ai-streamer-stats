...
> 2026-09-14 本地实现更新：老板工作台经营回顾默认昨天，显式传 `bizDate=yesterday`；原无日期接口和 AI 查询今天的语义保留。日期、完整性与下钻约定见[经营回顾日期口径](./老板工作台经营回顾日期口径.md)。下方历史盘点保留原时点含义。

> 历史盘点说明（2026-09-07）：正文记录 2026-08-20 的页面、接口与 AI 状态，其“已落地／待实现”不能直接代表当前仓库。新立项和 KPI 提交已改为负责人路径；旧审批按历史兼容核查。下一阶段所有页面、API 和 AI 状态／权限调整以[标准产品蓝图](./标准产品蓝图.md)及[实施清单](./标准产品-改造差距与实施清单.md)为设计入口，保留原角色作用域，不按文中具体账号建立通用产品规则。
