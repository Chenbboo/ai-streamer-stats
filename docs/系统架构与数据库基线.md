...
当前仍使用一个 Vue 3 前端、一个 Spring Boot/RuoYi 后端和共享 MySQL、Redis、身份与审计能力；没有拆成三个独立部署。通用业务分项目管理、人力资源管理、财务与管理核算三个系统；原直播、珠宝继续作为独立行业域。

```mermaid
flowchart LR
    UI[Vue 3 统一入口] --> PM[项目管理：计划 / 工作 / KPI / 交付]
    UI --> HR[人力资源：人员 / 奖励 / 假勤查询]
    UI --> FIN[财务与管理核算：费率 / 成本 / 关账]
    PM --> API[同一 RuoYi 业务服务与授权层]
    HR --> API
    FIN --> API
    IND[直播与珠宝行业页面] --> API
    API --> DB[MySQL：按业务对象保存来源与版本]
    API --> CACHE[Redis：登录与缓存]
    FEISHU[飞书只读 API] --> SOURCE[来源适配 / 映射 / 同步修订]
    SOURCE --> API
```

| 层次 | 当前代码 / 数据责任 |
| --- | --- |
| 通用前端 | `frontend/src/views/business`；页面挂入 `/business`、`/hcm`、`/finance`，集成挂入 `/platform` |
| API 与服务 | `web/controller/business`、`com.ruoyi.business`、`mapper/business`；操作权限与对象范围/责任校验同时执行 |
| P1 生命周期 | `biz_project` 保存交付、核算、结算确认及成本方法版本；项目关账不等于企业会计期间 |
| P2 计划与实际 | 模板/日历/单位政策、基线/变更/预测、资源/逐日容量、工作/审计/事件/成本；确认事件驱动计价，缺费率不伪造金额 |
| P3 奖励 | `biz_incentive_rule/award/event` 与 `biz_operating_fact` 来源关联；核准和成本确认独立，唯一来源避免重复入账 |
| P4 外部来源 | `biz_feishu_*` 保存连接、有效身份映射、批次、修订、问题、验收与读取范围；密钥只在服务端配置 |
| 行业边界 | `live_*` 与 `jewelry_*` 保持原始事实、成本及库存权威，不被通用管理流程反写 |

数据库在 V062 之后按 V063–V067 递增升级。已在本机复制库验证重复执行并应用 `localhost / ry-vue`，已有业务列哈希不变；未迁移生产。新项目基线与首快照同为 1，旧基线默认 0；费率分母独立保存，日/小时政策月度天数允许 null。真实部署与飞书切换仍需各自验收。

权限、接口、验证及迁移证据见 [P1–P4 总记录](./P1-P4-三系统实施与验收记录.md) 和 [迁移说明](../backend/sql/migrations/README.md)。下方保留早期技术基线，不能从旧“两业务域”描述推断现在只有两个业务域。

## 2026-08-04：历史架构快照

> 以下业务域、表结构及“当前”指 2026-08-04；技术约定按适用部分继续参考，冲突业务边界以上方增量架构为准。
