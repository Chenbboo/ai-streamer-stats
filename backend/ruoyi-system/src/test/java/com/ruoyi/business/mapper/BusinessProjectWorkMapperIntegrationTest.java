...
class BusinessProjectWorkMapperIntegrationTest
{
    ...
    @BeforeEach void setup()throws Exception
    {
        ...
        try(Connection c=source.getConnection();Statement s=c.createStatement())
        {
            ...
            for(String column:Arrays.asList("project_name varchar(160)","parent_funding_amount decimal(20,2)","parent_funding_reason varchar(1000)","company_dept_id bigint","department_id bigint","actual_start_date date","cost_policy_version varchar(32)","base_currency varchar(3)","del_flag char(1)","sponsor_owner_user_id bigint","initiator_user_id bigint"))s.execute("alter table biz_project add "+column);
        ...
        for(String name:Arrays.asList("BusinessProjectWorkMapper","BusinessProjectMapper")){String resource="mapper/business/"+name+".xml";try(InputStream in=Resources.getResourceAsStream(resource)){com.ruoyi.business.CompanyAccessTestSupport.register(config);new XMLMapperBuilder(in,config,resource,config.getSqlFragments()).parse();}}factory=new SqlSessionFactoryBuilder().build(config);
    }
