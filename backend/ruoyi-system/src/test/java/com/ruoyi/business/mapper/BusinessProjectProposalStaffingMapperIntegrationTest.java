    ...
    @Test void selectedModesSurviveRealSqlSaveAndReload() throws Exception {
        ...
        try(Connection c=source.getConnection();Statement sql=c.createStatement()){
            sql.execute("create table biz_project_proposal_staffing (staffing_line_id bigint auto_increment primary key,proposal_id bigint,"
                ...
                +"input_unit varchar(20),input_quantity decimal,allocation_plan_json clob,calendar_id bigint,unit_policy_id bigint,sort_order int,create_time timestamp)");
        }
        ...
        try(SqlSession session=factory.openSession()){
            ...
            for(int i=0;i<modes.length;i++){
                ...
                if(i==0)row.put("allocationPlanJson","{\"effectiveDate\":\"2026-09-08\",\"allocations\":[]}");
            ...
            assertTrue(String.valueOf(rows.get(0).get("allocationPlanJson")).contains("effectiveDate"));
        }
    }
}
