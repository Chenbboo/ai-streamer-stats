...
class BusinessFlowRegressionTest {
 ...
 BusinessAccountingServiceImpl as=new BusinessAccountingServiceImpl();{org.springframework.test.util.ReflectionTestUtils.setField(as,"companyAccess",com.ruoyi.business.CompanyAccessTestSupport.sponsorFixture());}
 ...
 BusinessFlowService flow=new BusinessFlowService();{org.springframework.test.util.ReflectionTestUtils.setField(flow,"companyAccess",com.ruoyi.business.CompanyAccessTestSupport.sponsorFixture());}
 ...
 @Test void routineOnlyCloseStillChecksPendingWork(){BusinessProjectServiceImpl service=new BusinessProjectServiceImpl();{org.springframework.test.util.ReflectionTestUtils.setField(service,"companyAccess",com.ruoyi.business.CompanyAccessTestSupport.sponsorFixture());}BusinessProjectWorkMapper work=mock(BusinessProjectWorkMapper.class);ReflectionTestUtils.setField(service,"mapper",pm);ReflectionTestUtils.setField(service,"workMapper",work);when(pm.selectRoutines(eq(1L),any())).thenReturn(Collections.singletonList(new BusinessProjectRoutine()));assertDoesNotThrow(()->ReflectionTestUtils.invokeMethod(service,"ensureReadyForAcceptance",1L));when(work.countPendingWork(1L)).thenReturn(1);assertThrows(ServiceException.class,()->ReflectionTestUtils.invokeMethod(service,"ensureReadyForAcceptance",1L));}
 @Test void emptyProjectCannotClose(){BusinessProjectServiceImpl service=new BusinessProjectServiceImpl();{org.springframework.test.util.ReflectionTestUtils.setField(service,"companyAccess",com.ruoyi.business.CompanyAccessTestSupport.sponsorFixture());}ReflectionTestUtils.setField(service,"mapper",pm);assertThrows(ServiceException.class,()->ReflectionTestUtils.invokeMethod(service,"ensureReadyForAcceptance",1L));}
