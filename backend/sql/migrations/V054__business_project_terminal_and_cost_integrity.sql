...
drop temporary table if exists tmp_v054_budget_spent;
create temporary table tmp_v054_budget_spent (
  result_id bigint not null primary key,
  budget_spent decimal(20,4) not null
) engine=InnoDB;

insert into tmp_v054_budget_spent(result_id,budget_spent)
...
join tmp_v054_budget_spent repaired on repaired.result_id=result_row.result_id
...
set routine.status='VOID',routine.update_by='migration-v054',routine.update_time=sysdate(),
...
set allocation.status=case when allocation.effective_from>project.actual_end_date then 'VOID' else allocation.status end,
  ...
  allocation.update_by='migration-v054',allocation.update_time=sysdate(),allocation.version=allocation.version+1
...
set task.status='CANCELED',task.update_by='migration-v054',task.update_time=sysdate(),task.version=task.version+1
