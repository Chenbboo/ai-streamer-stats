...
alter table biz_staff_leave convert to character set utf8mb4 collate utf8mb4_0900_ai_ci;

set @sql = if(
  (select count(*) from information_schema.columns where table_schema=database()
    and table_name='biz_staff_leave' and column_name='source_request_id')=0,
  "alter table biz_staff_leave add column source_request_id bigint default null comment 'approved leave request id' after recorded_project_id",
  'select 1');
prepare stmt from @sql; execute stmt; deallocate prepare stmt;

set @sql = if(
  (select count(*) from information_schema.statistics where table_schema=database()
    and table_name='biz_staff_leave' and index_name='idx_staff_leave_source_request')=0,
  'alter table biz_staff_leave add key idx_staff_leave_source_request (source_request_id)',
  'select 1');
prepare stmt from @sql; execute stmt; deallocate prepare stmt;

set @sql = if(
  (select count(*) from information_schema.columns where table_schema=database()
    and table_name='biz_staff_leave_request' and column_name='cancel_reviewed_user_id')=0,
  'alter table biz_staff_leave_request add column cancel_reviewed_user_id bigint default null after cancel_reason',
  'select 1');
prepare stmt from @sql; execute stmt; deallocate prepare stmt;

set @sql = if(
  (select count(*) from information_schema.columns where table_schema=database()
    and table_name='biz_staff_leave_request' and column_name='cancel_reviewed_user_name')=0,
  'alter table biz_staff_leave_request add column cancel_reviewed_user_name varchar(100) default null after cancel_reviewed_user_id',
  'select 1');
prepare stmt from @sql; execute stmt; deallocate prepare stmt;

set @sql = if(
  (select count(*) from information_schema.columns where table_schema=database()
    and table_name='biz_staff_leave_request' and column_name='cancel_reviewed_time')=0,
  'alter table biz_staff_leave_request add column cancel_reviewed_time datetime default null after cancel_reviewed_user_name',
  'select 1');
prepare stmt from @sql; execute stmt; deallocate prepare stmt;

set @sql = if(
  (select count(*) from information_schema.columns where table_schema=database()
    and table_name='biz_staff_leave_request' and column_name='cancel_review_comment')=0,
  'alter table biz_staff_leave_request add column cancel_review_comment varchar(1000) default null after cancel_reviewed_time',
  'select 1');
prepare stmt from @sql; execute stmt; deallocate prepare stmt;
...
set task.assignee_user_id=null,task.assignee_name=null,task.update_by='migration-v051',task.update_time=sysdate(),task.version=task.version+1
...
set routine.status='VOID',routine.update_by='migration-v051',routine.update_time=sysdate(),routine.version=routine.version+1
...
set allocation.status=case when allocation.effective_from>curdate() then 'VOID' else allocation.status end,
    ...
    allocation.update_by='migration-v051',allocation.update_time=sysdate(),allocation.version=allocation.version+1
