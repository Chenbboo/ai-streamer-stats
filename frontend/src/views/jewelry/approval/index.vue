<template>
  <div class="app-container">
    ...
    <el-table :data="rows" v-loading="loading" border>
      ...
      <el-table-column label="业务对象" min-width="150"><template #default="{row}">{{row.supplierNameSnapshot || row.salesChannel || (row.docType==='ASSEMBLY'?'手工组装成品':'—')}}</template></el-table-column>
      ...
      <el-table-column label="毛利" width="120" align="right"><template #default="{row}"><span v-if="row.docType==='ASSEMBLY'">—</span><el-button v-else link class="profit-link" :class="{loss:Number(row.totalProfit)<0}" title="查看毛利计算明细" @click="showProfit(row)">{{money(row.totalProfit)}}</el-button></template></el-table-column>
    ...
    <el-drawer v-model="drawer" title="单据明细" size="75%">
      <el-descriptions v-if="detail" :column="4" border>
        ...
        <el-descriptions-item :label="detail.docType==='ASSEMBLY'?'组装总成本':'总金额'">¥ {{money(detail.docType==='ASSEMBLY'?detail.totalCost:detail.totalAmount)}}</el-descriptions-item>
        <el-descriptions-item v-if="detail.docType!=='ASSEMBLY'" label="总成本">¥ {{money(detail.totalCost)}}</el-descriptions-item>
        <el-descriptions-item label="总毛利"><span v-if="detail.docType==='ASSEMBLY'">—</span><span v-else :class="{loss:Number(detail.totalProfit)<0}">¥ {{money(detail.totalProfit)}}</span></el-descriptions-item>
        <el-descriptions-item label="审核人">{{detail.secondReviewerName || detail.firstReviewerName || '—'}}</el-descriptions-item>
      </el-descriptions>
      ...
      <el-table v-if="detail" :data="detail.items" border class="mt20">
        ...
        <el-table-column :label="detail.docType==='ASSEMBLY'?'成本金额':'金额'"><template #default="{row}">{{money(detail.docType==='ASSEMBLY'?row.costAmount:row.amount)}}</template></el-table-column>
        <el-table-column label="毛利"><template #default="{row}"><span v-if="detail.docType==='ASSEMBLY'">—</span><span v-else :class="{loss:Number(row.profitAmount)<0}">{{money(row.profitAmount)}}</span></template></el-table-column>
...
const typeLabels={PURCHASE_IN:'采购入库',SALES_OUT:'销售出库',SUPPLIER_RETURN:'供应商退货',CUSTOMER_RETURN:'客户退货',RETURN_INSPECT:'退货质检',STOCK_ADJUST:'库存调整',ASSEMBLY:'手工组装',REVERSAL:'红冲单'}
