      ...
      <div class="sheet">
        ...
        <div v-if="excelImportSupported && !readonly" class="item-toolbar">
          ...
          <div class="item-toolbar-actions">
            ...
            <el-upload action="#" accept=".xls,.xlsx" :auto-upload="false" :show-file-list="false"
...
import {listJewelryDocuments,getJewelryDocument,saveJewelryDocument,submitJewelryDocument,withdrawJewelryDocument,createJewelryReversal,listJewelryProducts,listJewelrySuppliers,saveJewelryProduct,downloadJewelryDocumentImportTemplate,previewJewelryDocumentImport} from '@/api/jewelry/erp'
...
async function reloadProducts(){const r=await listJewelryProducts({pageNum:1,pageSize:500,status:'0'});products.value=r.rows||[]}
    ...
    for(const row of importPreview.value.rows||[]){
      if(row.newProduct){
        await saveJewelryProduct({
          sku:row.sku,productName:row.productName,category:row.category||'',
