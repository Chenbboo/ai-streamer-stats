<template>
  <div class="app-container">
    ...
    <el-dialog v-model="dialog" :title="readonly?'查看单据':(form.documentId?'编辑单据':'新建单据')" width="94%" top="4vh" destroy-on-close>
      <div class="sheet">
        <el-alert v-if="form.docType === 'REVERSAL'" title="红冲单明细来自原单，不允许修改；提交后仍需两级审批。" type="warning" :closable="false" show-icon />
...
import {listJewelryDocuments,getJewelryDocument,saveJewelryDocument,submitJewelryDocument,withdrawJewelryDocument,createJewelryReversal,listJewelryProducts,listJewelrySuppliers,saveJewelryProduct} from '@/api/jewelry/erp'
...
const blankQuickProduct=()=>({sku:'',productName:'',category:'',specification:'',unit:'件',warningQty:5,status:'0',defaultPackFee:0,defaultShipFee:0,defaultCertFee:0})
...
const types=[{value:'PURCHASE_IN',label:'采购入库'},{value:'SALES_OUT',label:'销售出库'},{value:'SUPPLIER_RETURN',label:'供应商退货'},{value:'CUSTOMER_RETURN',label:'客户退货'},{value:'RETURN_INSPECT',label:'退货质检'},{value:'STOCK_ADJUST',label:'库存调整'},{value:'REVERSAL',label:'红冲单'}]
const editableTypes=types.filter(item=>item.value!=='REVERSAL')
const statuses=[{value:'DRAFT',label:'草稿'},{value:'PENDING_FIRST',label:'待一审'},{value:'PENDING_SECOND',label:'待复核'},{value:'POSTED',label:'已入账'},{value:'REJECTED',label:'已驳回'},{value:'REVERSED',label:'已红冲'}]
const query=reactive({pageNum:1,pageSize:10,docNo:'',docType:'',status:''})
const blankItem=()=>({productId:null,qty:1,goodQty:0,defectQty:0,systemQty:0,countedQty:0,adjustmentQty:0,unitPrice:0,unitCost:0,packFee:0,shipFee:0,certFee:0,lineReason:''})
...
async function preload(){const [p,s,d]=await Promise.all([listJewelryProducts({pageNum:1,pageSize:500,status:'0'}),listJewelrySuppliers({pageNum:1,pageSize:500,status:'0'}),listJewelryDocuments({pageNum:1,pageSize:500,docType:'SALES_OUT',status:'POSTED'})]);products.value=p.rows||[];suppliers.value=s.rows||[];salesDocuments.value=d.rows||[]}
...
async function saveQuickProduct(){await productFormRef.value.validate();productSaving.value=true;try{await saveJewelryProduct(quickProduct);const r=await listJewelryProducts({pageNum:1,pageSize:500,status:'0'});products.value=r.rows||[];const created=products.value.find(p=>p.sku===quickProduct.sku);if(!created)throw new Error('商品已保存，但未能重新加载，请刷新后选择');activeProductRow.value.productId=created.productId;productChanged(activeProductRow.value);productDialog.value=false;proxy.$modal.msgSuccess('商品已新增并自动选中')}finally{productSaving.value=false}}
...
function typeChanged(){form.items=[blankItem()];form.supplierId=null;form.supplierNameSnapshot='';form.salesChannel='';form.platformRate=0;form.commissionRate=0;form.taxRate=0;form.returnReason='';form.sourceDocumentId=null;form.unlinkedReason=''}
...
async function reverse(row){await proxy.$modal.confirm(`确认对单据 ${row.docNo} 发起整单红冲？红冲单还需要两级审批。`);await createJewelryReversal(row.documentId);proxy.$modal.msgSuccess('红冲草稿已生成');load()}
preload();load()
</script>
<style scoped>.sheet{border:1px solid #cfd5dc}.sheet-head{display:grid;grid-template-columns:repeat(6,minmax(150px,1fr));gap:12px;padding:14px;background:#f4f6f8}.sheet-head :deep(.el-form-item){margin:0}.sheet-head :deep(.el-input-number),.sheet-head :deep(.el-select),.sheet-head :deep(.el-date-editor){width:100%}.item-table{border-left:0;border-right:0}.item-table :deep(.el-input-number){width:100%;min-width:0}.product-picker{display:flex;align-items:center;gap:8px}.product-picker .el-select{flex:1;min-width:0}.product-picker .el-button{flex:none}.add-line{margin:12px}.document-total{display:flex;justify-content:flex-end;gap:28px;padding:12px 16px;border-top:1px solid #d9dee5;background:#f8fafc;color:#475569}.document-total b{color:#111827}.loss,.document-total .loss,.document-total .loss b{color:#dc2626;font-weight:700}.sheet-foot{padding:12px 14px 0;border-top:1px solid #d9dee5}@media(max-width:1200px){.sheet-head{grid-template-columns:repeat(3,1fr)}}@media(max-width:760px){.sheet-head{grid-template-columns:1fr}.product-picker{align-items:stretch;flex-direction:column}.document-total{justify-content:flex-start;flex-wrap:wrap;gap:12px 20px}}</style>
