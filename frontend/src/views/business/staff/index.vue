<template>
  <div class="app-container staff-page">
    ...
    <el-dialog v-model="costDialog" title="设置月度用人成本" width="min(580px, 94vw)" append-to-body><el-alert title="仅用于公司内部项目核算，不代表员工工资单。每次保存都会产生新版本，历史成本不会被覆盖。" type="warning" :closable="false" show-icon/><el-form :model="costForm" label-width="126px" class="cost-form"><el-form-item label="人员"><el-input :model-value="selectedPerson ? `${selectedPerson.nickName}（${selectedPerson.userName}）` : ''" disabled /></el-form-item><el-form-item label="国家/地区"><el-input :model-value="regionLabel(selectedPerson?.countryRegion)" disabled /></el-form-item><el-form-item label="月度用人成本" required><el-input-number v-model="costForm.unitCost" :min="0" :precision="2" :step="100" controls-position="right" style="width:100%" /><div class="form-help">统一使用人民币（CNY）填写。</div></el-form-item><div class="cost-preview"><span>系统折算</span><b>{{ money(costForm.unitCost) }} ÷ {{ selectedStandardDays || '—' }} 天 = {{ calculatedDailyCost }} 元/天</b><small>{{ selectedPerson?.countryRegion==='VN' ? '越南员工按 26 天' : selectedPerson?.countryRegion==='CN' ? '中国员工按 21.75 天' : '该国家/地区尚未配置折算规则' }}</small></div><el-form-item label="生效日期" required><el-date-picker v-model="costForm.effectiveFrom" type="date" value-format="YYYY-MM-DD" style="width:100%" /></el-form-item><el-form-item label="失效日期"><el-date-picker v-model="costForm.effectiveTo" type="date" value-format="YYYY-MM-DD" clearable style="width:100%" /></el-form-item><el-form-item label="调整说明"><el-input v-model="costForm.remark" type="textarea" :rows="3" maxlength="500" show-word-limit placeholder="例如：转正后调整月度用人成本" /></el-form-item></el-form><template #footer><el-button @click="costDialog=false">取消</el-button><el-button type="primary" :loading="saving" @click="saveCostPolicy">保存成本版本</el-button></template></el-dialog>
  </div>
</template>
...
const loading=ref(false),saving=ref(false),dialogOpen=ref(false),detailOpen=ref(false),projectLoading=ref(false),deepLinkHandled=ref(false)
