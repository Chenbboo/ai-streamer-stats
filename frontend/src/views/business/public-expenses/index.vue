<template>
  <div class="app-container public-expenses-page">
    <header class="page-header">
      <div><h1>公司公共费用</h1><p>填好费用，分摊给负责人，月份结束后结算。</p></div>
    ...
    <section class="expense-panel filters-panel">
      <el-form inline label-position="top" @submit.prevent>
        ...
        <el-form-item label="分摊月份"><el-date-picker v-model="filters.month" type="month" value-format="YYYY-MM" :clearable="false" :disabled="saving || loading" @change="changeFilters" /></el-form-item>
      ...
      <span class="filter-note">下面填写和分摊的是所选公司 {{ filters.month }} 的费用。部门和负责人随公司切换，各公司、月份及币种分别结算。</span>
    </section>
    ...
    <div v-else v-loading="loading" class="workspace">
      ...
      <nav class="flow-steps" aria-label="费用办理步骤">
        <button v-for="(label, index) in ['核对费用', '分摊给负责人', '分摊进度与月结']" :key="label" type="button" :aria-current="step === index + 1 ? 'step' : undefined" :class="{ current: step === index + 1 }" :disabled="saving || loading || (index === 1 && !bill) || (index === 2 && (!bill || bill.status === 'DRAFT'))" @click="selectStep(index + 1)"><span>{{ index + 1 }}</span>{{ label }}</button>
          ...
          <div v-if="step === 1 && costTab === 'PERSONNEL' && bill?.personnel" class="table-footer"><span>两类费用分别设置分摊比例，统一下发和月结。</span><el-button type="primary" :disabled="saving || loading" @click="costPool = 'PERSONNEL'; restoreOwners(); selectStep(2)">查看负责人分摊</el-button></div>
          <section v-if="step === 1 && costTab === 'EXPENSE'" class="expense-panel fees-panel">
            <div class="section-header"><div><h2>{{ filters.month }} 费用明细</h2><p>{{ bill ? '核对本月金额。尚未确认的费用可以先分摊，月结前再确认。' : '已自动带出本月适用的常用费用，确认后即可分摊。' }}</p></div><div class="actions"><el-button v-if="(!bill || bill.status === 'DRAFT') && canManage" icon="Plus" :disabled="saving || loading" @click="openPolicy()">添加费用</el-button><el-button v-if="bill?.status === 'DRAFT' && canManage && missingPoliciesCount" :disabled="dirty || loading" :loading="saving" @click="syncNewPolicies">加入新增费用（{{ missingPoliciesCount }}）</el-button><el-button v-if="bill?.status === 'PUBLISHED' && canManage" :disabled="loading || saving" @click="recallMonth">退回修改</el-button><el-button v-if="bill?.status === 'SETTLED' && canManage" :disabled="loading || saving" @click="openAdjustment">登记调整</el-button></div></div>
            <template v-if="!bill">
              <el-table v-if="monthPolicies.length" :data="monthPolicies" empty-text="本月没有费用">
                ...
                <el-table-column label="金额状态" min-width="120"><template #default="{ row }"><el-tag :type="row.estimated ? 'warning' : 'success'">{{ row.estimated ? '暂估，待确认' : '已确认' }}</el-tag></template></el-table-column>
              ...
              <div class="table-footer"><p>年费自动分到 12 个月；常用费用在有效期内自动带出。</p><el-button v-if="canManage" type="primary" :disabled="!monthPolicies.length || loading" :loading="saving" @click="generateMonth">下一步：分摊给负责人</el-button></div>
            </template>
            <template v-else>
              <el-alert v-if="bill.status === 'SETTLED'" title="本月已结算并锁定。后续变动请登记调整，原费用和分摊记录继续保留。" type="success" :closable="false" class="section-gap" />
              <el-alert v-if="estimatedCount" :title="`有 ${estimatedCount} 笔金额待确认。可先分摊；下发后要修改，需退回并重新下发。`" type="warning" :closable="false" show-icon class="section-gap" />
              <el-table :data="entryRows" row-key="entryId" empty-text="本月没有费用明细">
                ...
                <el-table-column :label="`本月金额（${filters.currency}）`" min-width="170" align="right"><template #default="{ row }"><el-input-number v-if="canEditMonth" v-model="row.amount" :min="0" :max="999999999999" :precision="2" :controls="false" aria-label="本月金额" /><b v-else>{{ money(row.amount) }}</b></template></el-table-column>
                <el-table-column label="金额状态" min-width="150"><template #default="{ row }"><el-select v-if="canEditMonth" v-model="row.estimated" aria-label="金额状态"><el-option :value="false" label="已确认" /><el-option :value="true" label="暂估，待确认" /></el-select><el-tag v-else :type="row.estimated ? 'warning' : 'success'">{{ row.estimated ? '暂估，待确认' : '已确认' }}</el-tag></template></el-table-column>
              ...
              <div class="table-footer"><span>费用合计：<b>{{ money(entryTotal) }} {{ filters.currency }}</b><em v-if="entriesDirty"> · 明细尚未保存</em></span><div v-if="bill.status === 'DRAFT' && canManage" class="actions"><el-button v-if="entriesDirty" :disabled="saving || loading" @click="restoreEntries">还原</el-button><el-button v-if="entriesDirty" :disabled="ownersDirty || loading" :loading="saving" @click="saveEntries">暂存费用</el-button><el-button type="primary" :disabled="ownersDirty || loading" :loading="saving" @click="continueToOwners">下一步：分摊给负责人</el-button></div></div>
          ...
          <section v-if="bill && step > 1" class="expense-panel allocation-panel">
            <div class="section-header"><div><h2>{{ step === 2 ? '这些费用由谁承担？' : '项目分摊进度' }}</h2><p>{{ bill.status === 'DRAFT' ? '填写各负责人的分摊比例，合计 100% 后即可下发到工作台。' : '负责人在自己的工作台分摊到项目并提交，你在这里查看进度和月结。' }}</p></div><div v-if="canEditMonth" class="actions"><el-button :disabled="saving || dirty" @click="copyOwners">沿用上月比例</el-button><el-button icon="Plus" :disabled="saving" @click="addOwner">添加负责人</el-button></div><div v-else class="actions"><el-button v-if="bill.status === 'PUBLISHED' && canManage" :disabled="saving || loading" @click="recallMonth">退回修改</el-button><el-button v-if="bill.status === 'SETTLED' && canManage" :disabled="saving || loading" @click="openAdjustment">登记调整</el-button></div></div>
            ...
            <p class="pool-note">{{ costPool === 'PERSONNEL' ? '按部门选择负责人分摊公共人员成本；日暂估金额为月分摊金额 ÷ 21.75。' : '日常公共费用按独立比例分摊。' }} 两类费用均分摊完成后统一下发。</p>
            <div v-if="step === 3" class="settlement-bar"><div><b>{{ bill.status === 'SETTLED' ? '本月已结算' : '确认月结' }}</b><p>{{ bill.status === 'SETTLED' ? '公共费用已计入项目成本。后续增减可登记调整，原记录保留。' : settleNotice }}</p><p v-if="bill.status === 'PUBLISHED'">待分摊到项目 {{ money(projectRemainingAmount) }} {{ filters.currency }} · {{ pendingOwnerCount }} 位负责人待提交</p></div><el-button v-if="bill.status === 'PUBLISHED' && canManage" type="primary" :disabled="!canSettle || loading" :loading="saving" @click="settleMonth">确认月结</el-button></div>
            <el-table :data="ownerRows" empty-text="请添加负责人并填写分摊比例">
              ...
              <el-table-column label="分摊比例" min-width="155"><template #default="{ row }"><span v-if="canEditMonth" class="percentage-field"><el-input-number v-model="row.percentage" :min="0" :max="100" :precision="2" :controls="false" aria-label="负责人分摊比例" /> %</span><span v-else>{{ row.percentage }}%</span></template></el-table-column>
              <el-table-column :label="`月分摊金额（${filters.currency}）`" min-width="150" align="right"><template #default="{ row, $index }">{{ money(canEditMonth && ownersDirty ? ownerAmountPreview($index) : row.amount) }}</template></el-table-column>
              <el-table-column v-if="costPool === 'PERSONNEL'" label="日暂估金额（÷ 21.75）" min-width="145" align="right"><template #default="{ row, $index }">{{ money((canEditMonth && ownersDirty ? ownerAmountPreview($index) : row.amount) / 21.75) }}</template></el-table-column>
              <el-table-column label="项目分摊" min-width="170"><template #default="{ row }"><template v-if="bill.status !== 'DRAFT'"><el-tag :type="row.status === 'SUBMITTED' ? 'success' : 'warning'">{{ row.status === 'SUBMITTED' ? '已提交' : '待提交' }}</el-tag><small>待分摊 {{ money(row.remainingAmount) }}</small></template><span v-else>下发后由负责人分摊</span></template></el-table-column>
              ...
              <el-table-column v-else type="expand"><template #default="{ row }"><el-table class="project-details" :data="row.projects || []" empty-text="负责人尚未分摊到项目"><el-table-column prop="projectName" label="项目" /><el-table-column label="分摊比例"><template #default="{ row: project }">{{ project.percentage }}%</template></el-table-column><el-table-column label="月分摊金额"><template #default="{ row: project }">{{ money(project.amount) }} {{ filters.currency }}</template></el-table-column></el-table></template></el-table-column>
            </el-table>
            ...
            <el-table v-if="costPool === 'PERSONNEL' && departmentTotals.length" :data="departmentTotals" class="section-gap"><el-table-column prop="name" label="部门分摊汇总"/><el-table-column label="月分摊金额" align="right"><template #default="{ row }">{{ money(row.amount) }} {{ filters.currency }}</template></el-table-column><el-table-column label="日暂估金额（÷ 21.75）" align="right"><template #default="{ row }">{{ money(row.amount / 21.75) }} {{ filters.currency }}</template></el-table-column></el-table>
    ...
    <el-dialog v-model="policyDialog" :title="policyForm.policyId ? '修改常用费用' : '添加费用'" width="min(660px, 95vw)" append-to-body :close-on-click-modal="false" :close-on-press-escape="!saving" :show-close="!saving">
      ...
      <el-form :model="policyForm" label-position="top" class="expense-form" :disabled="saving || policyReadOnly">
        ...
        <el-form-item label="金额状态"><el-radio-group v-model="policyForm.estimated"><el-radio-button :value="false">已确认</el-radio-button><el-radio-button :value="true">暂估，待确认</el-radio-button></el-radio-group></el-form-item>
...
const settleNotice = computed(() => {
  ...
  return issues.length ? issues.join('；') : '费用已确认，负责人已完成项目分摊，可确认本月结算。'
})
...
async function savePolicy() {
  if (!policyForm.name?.trim() || !policyForm.category || !Number.isFinite(Number(policyForm.amount)) || Number(policyForm.amount) <= 0) return ElMessage.warning('请核对费用名称、类别和大于 0 的金额。')
...
async function generateMonth() { if (await mutate(() => generatePublicExpenseMonth({ ...filters }), '本月费用已保存，请设置负责人分摊。')) step.value = 2 }
async function syncNewPolicies() { if (dirty.value) return ElMessage.warning('请先保存当前修改。'); await mutate(() => generatePublicExpenseMonth({ ...filters, syncNewPolicies: true, version: bill.value.version }), '已补充本月新增费用，原有账单金额继续保留，请核对负责人月分摊金额。') }
