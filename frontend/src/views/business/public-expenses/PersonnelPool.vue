<template>
  <section class="personnel-pool" v-loading="loading">
    <div class="heading"><div><h2>公共人员成本</h2><p>自动汇总人员月成本，扣除项目已承担金额；剩余成本按老板设置的部门、负责人比例分摊。</p></div><div class="actions"><el-button :disabled="disabled || saving" :loading="loading" @click="refreshPreview">刷新人员数据</el-button><el-button v-if="editable" type="primary" :disabled="disabled || loading || loadFailed || !previewLoaded || blocked" :loading="saving" @click="save">{{ snapshot ? '更新金额并设置分摊' : '下一步：设置分摊对象和比例' }}</el-button><el-button v-else-if="bill?.status === 'PUBLISHED'" type="primary" :disabled="disabled || saving" @click="emit('recall')">{{ snapshot ? '退回月账更新分摊' : '退回月账设置人员分摊' }}</el-button></div></div>
    <template v-if="snapshot">
      <div class="metrics"><div><span>公司人员月成本</span><b>{{ money(snapshot.totalAmount) }}</b></div><div><span>项目已直接承担</span><b>{{ money(Number(snapshot.projectAmount) + Number(snapshot.businessAmount)) }}</b></div><div><span>公共人员月成本</span><b>{{ money(snapshot.publicAmount) }} {{ filters.currency }}</b></div><div><span>日估算参考（月额 ÷ 21.75）</span><b>{{ money(snapshot.dailyReference) }}</b></div></div>
      <el-alert :type="snapshot.estimated ? 'warning' : 'success'" :closable="false" :title="snapshot.estimated ? '本月金额为整月预估；月底请刷新整月人员成本，再与日常公共费用一起月结。' : '整月人员成本已保存，将与日常公共费用一起月结。'" />
      <el-collapse><el-collapse-item title="查看人员成本来源（仅老板可见）"><el-table :data="snapshot.rows"><el-table-column prop="userName" label="人员" min-width="100"/><el-table-column prop="deptName" label="所属部门" min-width="110"/><el-table-column label="月总成本" min-width="120"><template #default="{ row }">{{ money(row.totalAmount) }}</template></el-table-column><el-table-column label="项目直接承担" min-width="130"><template #default="{ row }">{{ money(Number(row.projectAmount) + Number(row.businessAmount)) }}</template></el-table-column><el-table-column label="待分摊" min-width="120"><template #default="{ row }">{{ money(row.publicAmount) }}</template></el-table-column></el-table></el-collapse-item></el-collapse>
    </template>
    <div class="current-source">
      <el-alert v-if="!snapshot" :title="bill?.status === 'PUBLISHED' ? '本月账单已下发，尚未加入公共人员成本。下方为人员数据预览，未计入费用；加入前需退回月账，修改后重新下发。' : bill?.status === 'SETTLED' ? '本月账单已结算，未包含公共人员成本。下方仅供核对，不改动已结算账单。' : '金额自动读取，无需重复填写。点击“下一步”，选择部门、负责人及承担比例。'" type="info" :closable="false" show-icon/>
      <el-alert v-if="loadFailed" class="note" title="人员成本加载失败，请点击“刷新人员数据”重试。" type="error" :closable="false" show-icon/>
      <template v-else-if="previewLoaded">
        <div class="metrics preview-metrics"><div><span>人员数量</span><b>{{ rows.length }} 人</b></div><div><span>人员月成本（预览）</span><b>{{ money(previewPayroll) }}</b></div><div><span>待分摊月成本（预览）</span><b>{{ money(total) }}</b></div><div><span>日估算参考（÷ 21.75）</span><b>{{ money(total == null ? null : total / 21.75) }}</b></div></div>
        ...
        <el-table :data="rows" class="source-preview" max-height="440" empty-text="所选公司、月份和币种暂无适用人员。请核对人员归属及成本设置。">
          ...
          <el-table-column label="项目直接人员成本" min-width="160"><template #default="{ row }"><span>{{ row.projectIssues?.length ? '已测算：' : '' }}{{ money(row.projectAmount) }}</span><small v-if="row.projectIssues?.length" class="warning">部分日期待完善，非完整金额</small></template></el-table-column>
          ...
          <el-table-column label="待分摊成本" min-width="130"><template #default="{ row }">{{ money(row.projectIssues?.length ? null : remainder(row)) }}</template></el-table-column>
          <el-table-column label="核对状态" min-width="250"><template #default="{ row }"><div class="review-status"><el-tag v-if="row.issues?.length" type="warning" effect="plain">月成本待完善</el-tag><el-tag v-if="row.projectIssues?.length" type="warning" effect="plain">项目成本待完善</el-tag><span v-if="!row.issues?.length && !row.projectIssues?.length">已自动计算</span><el-button v-else link type="primary" @click="showIssues(row)">查看原因</el-button></div></template></el-table-column>
        </el-table>
      </template>
    ...
    <p class="note">人员金额只读；需要调整时请到人员成本设置修改。公共人员成本单独设置部门、负责人比例。负责人分配到项目后，日结果按月承担额 ÷ 21.75 暂估；月结以实际月额替换估算，不重复扣费。</p>
...
const issueGroups = computed(() => {
  ...
  return [
    ...
    { title: '项目直接承担成本', items: row.projectIssueDetails || (row.projectIssues || []).map(reason => ({ reason })), help: '请在对应项目的人员工作日成本中补齐缺项；涉及投入分配的，需由相关负责人确认。完善后刷新人员数据。' }
...
async function save() {
  ...
  try {
    ...
    ElMessage.success('已自动汇总人员成本，请设置部门、负责人和承担比例。')
    emit('saved')
