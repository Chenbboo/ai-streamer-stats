    ...
    <section class="panel pending-panel">
      ...
      >
        ...
        <div v-else class="decision-copy">
          <div class="decision-title">
            <b>{{ row.userName || row.projectName }}</b>
...
const pendingLabel = row => row.category === 'PROPOSAL' ? '立项待审批' : row.category === 'KPI_MISSING' ? (Number(row.targetCount) ? 'KPI 待发布' : 'KPI 待设置') : row.category === 'KPI_REVIEW' ? 'KPI 结算待确认' : '项目状态待处理'
const pendingDotClass = row => row.category === 'KPI_MISSING' ? 'dot-danger' : row.category === 'PERSONNEL_COST_GROUP' || row.category === 'PROPOSAL' || row.category === 'KPI_REVIEW' ? 'dot-warning' : 'dot-info'
const pendingBadgeClass = row => row.category === 'KPI_MISSING' ? 'badge-danger' : row.category === 'PROPOSAL' || row.category === 'KPI_REVIEW' ? 'badge-warning' : 'badge-info'
const pendingDescription = row => row.category === 'PROPOSAL' ? (row.objective || '新的立项申请等待审批') : row.category === 'KPI_MISSING' ? (Number(row.targetCount) ? `已有 ${row.targetCount} 项 KPI 目标，但尚未发布考核与奖金方案` : '项目已进入执行流程，KPI 目标待设置') : row.category === 'KPI_REVIEW' ? '负责人已提交 KPI 结果，确认后项目奖金会立即计入成本' : decisionHint(row)
