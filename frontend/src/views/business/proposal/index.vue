  ...
  <div class="proposal-page">
    ...
    <el-dialog v-model="formVisible" :title="form.proposalId ? '修改立项申请' : '新建立项申请'" width="min(1180px,97vw)" append-to-body>
      <el-form ref="formRef" class="proposal-form" :model="form" :rules="rules" label-width="112px" require-asterisk-position="left" scroll-to-error :scroll-into-view-options="{block:'center',behavior:'smooth'}">
        <p class="required-hint"><span>*</span> 为必填项；明细区是否必填以标题标记为准，添加明细后请填写带星号的列。</p>
        ...
        <el-row :gutter="16">
          ...
          <el-col :span="24"><el-form-item label="目标模式" prop="goalMode" required><el-radio-group v-model="form.goalMode"><el-radio value="TOTAL">按项目总目标跟踪</el-radio><el-radio value="NO_TOTAL">持续经营，按每日目标与成果跟踪</el-radio></el-radio-group></el-form-item></el-col><el-col :span="24"><el-form-item label="项目目标" prop="objective"><el-input v-model="form.objective" type="textarea" :rows="3" maxlength="1000" show-word-limit placeholder="例明项目需要完成的指标、成果清单、通过条件和核验收人" /></el-form-item></el-col>
    ...
    <el-drawer v-model="detailVisible" size="min(720px,94vw)" append-to-body>
      ...
      <section v-if="detail.budget" class="detail-section budget-composition">
        ...
        <div class="budget-meta">
          <div><span>预算期间</span><b>{{ detail.budget.startDate }} 至 {{ detail.budget.endDate }}</b></div>
...
// Match existing save/launch requirements; accounting mode does not add requirements to new templates.
const requiredPlanSections=computed(()=>({
  staffing:!isNewTemplate.value,
  targets:['VALUE','HYBRID'].includes(form.value.accountingMode)||!isNewTemplate.value&&form.value.goalMode!=='NO_TOTAL',
  revenue:['PROFIT','HYBRID'].includes(form.value.accountingMode)
}))
const showTargetLines=computed(()=>form.value.goalMode!=='NO_TOTAL'||requiredPlanSections.value.targets)
