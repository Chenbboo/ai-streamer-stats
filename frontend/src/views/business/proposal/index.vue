...
const isChildCreatorPhase=computed(()=>!!form.value.parentProjectId&&(!form.value.proposalId||!isAssignedSubOwner.value))
const canConfigureStaffing=computed(()=>!isChildCreatorPhase.value)
const canLaunchForm=computed(()=>!form.value.parentProjectId||isAssignedSubOwner.value&&!isChildCreatorPhase.value)
...
const requiredPlanSections=computed(()=>({
  ...
  targets:!form.value.parentProjectId,
  revenue:false
}))
const showTargetLines=computed(()=>true)
...
let openingProposalFromRoute=false
...
watch(()=>[route.path,route.query.id,route.query.edit],openRequestedProposal)
onMounted(async()=>{activeTab.value=route.query.tab==='directory'&&canViewDirectory.value?'directory':'mine';await ensureOptions();await refreshAll();await openRequestedProposal();await openRequestedForm()})
useBusinessRefreshOnReactivated(async()=>{await refreshAll();await openRequestedProposal()})
</script>
...
.creator-funding-section{padding:22px}.child-funding-panel{border-color:#b8ddcb;background:#f3fbf7}.handoff-summary-section{background:linear-gradient(180deg,#f8fbff 0%,#fff 100%)}.handoff-summary-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.handoff-summary-grid>div{padding:12px;border:1px solid #e2eaf3;border-radius:8px;background:#fff}.handoff-summary-grid .wide{grid-column:span 2}.handoff-summary-grid span,.handoff-summary-grid b,.handoff-funding-card span,.handoff-funding-card b,.handoff-funding-card small{display:block}.handoff-summary-grid span,.handoff-funding-card span{color:#718096;font-size:12px}.handoff-summary-grid b,.handoff-funding-card b{margin-top:5px;color:#24364b;line-height:1.5}.handoff-funding-card{display:grid;grid-template-columns:minmax(220px,1fr) 2fr;gap:16px;margin-top:14px;padding:16px;border:1px solid #b8ddcb;border-radius:10px;background:#f1faf5}.handoff-funding-card b{color:#16945e;font-size:18px}.handoff-funding-card small{margin-top:4px;color:#718096}
