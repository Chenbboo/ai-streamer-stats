<template>
  <div class="proposal-page">
    ...
    <el-dialog v-model="formVisible" :title="form.proposalId ? '修改立项申请' : '新建立项申请'" width="min(1180px,97vw)" :before-close="beforeFormClose" append-to-body>
      <el-form ref="formRef" class="proposal-form" :disabled="saving" :model="form" :rules="rules" :validate-on-rule-change="false" label-width="112px" require-asterisk-position="left" scroll-to-error :scroll-into-view-options="{block:'center',behavior:'smooth'}">
        ...
        <el-row :gutter="16">
          ...
          <el-col :span="12"><el-form-item label="归属公司" prop="companyDeptId"><el-input :model-value="ownerCompany" readonly :placeholder="form.parentProjectId && !form.assignedOwnerUserId ? '选择负责人后自动带入' : '负责人未设置所属公司，请先完善人员资料'" /></el-form-item></el-col>
          ...
          <el-col :span="24"><el-form-item label="主要风险" prop="riskSummary"><el-input v-model="form.riskSummary" type="textarea" :rows="2" maxlength="2000" show-word-limit placeholder="选填，列出可能影响收入、成本和进度的风险" /></el-form-item></el-col>
        </el-row>
      </el-form>
      <template #footer><el-button @click="beforeFormClose()">取消</el-button><el-button :loading="saving" @click="saveForm(false)">保存草稿</el-button><el-button type="primary" :loading="saving" @click="saveForm(true)">启动项目</el-button></template>
    </el-dialog>
    ...
    <el-drawer v-model="detailVisible" size="min(720px,94vw)" append-to-body>
      ...
      <section v-if="detail.budget?.steadyMonth" class="detail-section"><h3>下月预计成本</h3><p>{{ detail.budget.steadyMonth.startDate }} 至 {{ detail.budget.steadyMonth.endDate }}，按立项人员计划和当前有效费率测算。月成本制整月按月度用人成本计入。</p><div class="finance-summary compact"><div><span>人员成本</span><b>{{ money(detail.budget.steadyMonth.personnelAmount,detail.baseCurrency) }}</b></div><div><span>业务成本</span><b>{{ money(detail.budget.steadyMonth.plannedBusinessAmount,detail.baseCurrency) }}</b></div><div><span>预计总成本</span><b>{{ money(detail.budget.steadyMonth.plannedTotalCost,detail.baseCurrency) }}</b></div></div><el-alert v-if="detail.budget.steadyMonth.status==='PENDING'" title="下月测算待完善" :description="detail.budget.steadyMonth.issues?.join('；')" type="warning" :closable="false" /></section>
...
const rules=computed(()=>({
  projectName:requiredRule('请输入项目名称','blur'),assignedOwnerUserId:form.value.parentProjectId?requiredRule('请选择子项目负责人'):[],sponsorOwnerUserId:form.value.parentProjectId?[]:requiredRule('请选择归属老板'),companyDeptId:requiredRule('负责人未设置有效所属公司，请先完善人员资料'),
  ...
  'budget.businessAmount':form.value.budget?.mode==='TOTAL'?amountRule('请填写有效业务预算，无支出填0'):[],'budget.cycle':openEnded.value?requiredRule('请选择预算周期'):[],'budget.anchorDate':openEnded.value?requiredRule(`请选择${budgetPeriodFieldLabel.value}`):[],riskSummary:[]
}))
...
const ownerCompany=computed(()=>options.companies.find(company=>String(company.deptId)===String(proposalOwner.value?.companyDeptId))?.deptName||'')
...
function changeAssignedOwner(){
  ...
  syncOwnerCompany();ensureOwnerStaff();refreshStaffOptions()
}
...
const requiredPlanSections=computed(()=>({
  ...
  targets:true,
  revenue:form.value.accountingMode==='PROFIT'
}))
const showTargetLines=computed(()=>requiredPlanSections.value.targets)
const showRevenueLines=computed(()=>requiredPlanSections.value.revenue||!!form.value.revenueLines?.length)
...
async function saveForm(launch=false){if(saving.value)return;saving.value=true;try{if(launch||!isNewTemplate.value){try{await formRef.value.validate()}catch{return;}const detailError=validateDetailLines();if(detailError)return showPlanError(detailError);if(!form.value.planStartDate)return ElMessage.warning('请选择计划开始日期');if(!openEnded.value&&!form.value.planEndDate)return ElMessage.warning('请选择计划结束日期或勾选不限期');if(form.value.planEndDate&&form.value.planStartDate>form.value.planEndDate)return ElMessage.warning('计划结束日期不能早于开始日期');if(form.value.managementMode==='KEY_CONTROL'&&!form.value.managementReason?.trim())return ElMessage.warning('重点监管项目请填写监管原因');const staffing=form.value.staffingLines||[];if(!isNewTemplate.value&&!staffing.length||staffing.some(row=>!row.userId))return ElMessage.warning('请在人员计划中选择具体人员');if(new Set(staffing.map(row=>String(row.userId))).size!==staffing.length)return ElMessage.warning('同一人员不能重复选择');if(isNewTemplate.value)for(const row of staffing){syncParticipation(row);if(!['FOLLOW_PROJECT','CUSTOM','UNLIMITED'].includes(row.participationMode)||!row.planStartDate||row.participationMode==='CUSTOM'&&!row.planEndDate||row.planEndDate&&row.planStartDate>row.planEndDate||row.planStartDate<form.value.planStartDate||!openEnded.value&&form.value.planEndDate&&row.planEndDate>form.value.planEndDate||row.participationMode==='UNLIMITED'&&!openEnded.value)return ElMessage.warning('请填写有效的人员参与方式和项目范围内的日期');if(!row.calendarId)return ElMessage.warning('当前没有覆盖人员参与期间的有效工作日历，请联系管理员配置')}if(launch){
  if(requiredPlanSections.value.revenue&&!form.value.revenueLines?.some(row=>row.scenario==='BASE'&&Number(row.expectedAmount)>0))return showPlanError('请填写预计收入，合计须大于0')
  if(!form.value.expenseLines?.length)return showPlanError('请填写至少一项业务支出计划')
...
form.value.proposalId=res.data.proposalId;form.value.version=res.data.version;formBaseline=formSnapshot();if(launch){try{const started=await submitProjectProposal(res.data.proposalId);ElMessage.success('项目已启动');formVisible.value=false;detailVisible.value=false;await refreshAll();if(started.data?.createdProjectId)openProject(started.data)}catch(error){await showPlanError(error?.message||'启动未完成，草稿已保存');await refreshAll()}}else{formVisible.value=false;ElMessage.success('草稿已保存，可继续完善后启动');await refreshAll();await openDetail(res.data)}return true}catch(error){return false}finally{saving.value=false}}
