<template>
  <div class="proposal-page">
        ...
        <el-card shadow="never">
          <el-table :data="mineRows" v-loading="loading" empty-text="还没有立项申请" @row-click="openDetail">
            ...
            <el-table-column label="操作" width="235" fixed="right"><template #default="{row}"><div class="row-actions" @click.stop>
              ...
              <el-button v-if="row.canEdit&&canLaunchProposal(row)" link type="success" @click="submitRow(row)">启动项目</el-button>
    ...
    <el-dialog v-model="formVisible" :title="form.proposalId ? '修改立项申请' : '新建立项申请'" width="min(1180px,97vw)" :before-close="beforeFormClose" append-to-body>
      <el-form ref="formRef" class="proposal-form" :disabled="saving" :model="form" :rules="rules" :validate-on-rule-change="false" label-width="112px" require-asterisk-position="left" scroll-to-error :scroll-into-view-options="{block:'center',behavior:'smooth'}">
        ...
        <el-row :gutter="16">
          ...
          <el-col v-if="canConfigureStaffing" :span="24"><div class="plan-section"><div class="plan-section-head"><div><h3 :class="{'required-section':requiredPlanSections.staffing}">人员与投入比例</h3><p>在立项阶段一次设置，启动后直接形成项目人员计划</p></div><el-button size="small" type="primary" plain @click="addStaffing">增加人员</el-button></div>
            <el-table :data="form.staffingLines" size="small" empty-text="暂无人员，可启动后添加">
                  ...
                  <el-popover placement="bottom-start" :width="720" trigger="click" @before-enter="loadStaffAllocationPlan(row,true)">
                    ...
                    <div v-loading="staffAllocationLoading(row)" class="allocation-popover">
                      <template v-if="staffPeriodProjects(row).length">
                        <div class="allocation-head"><div><b>{{ row.userName || '所选人员' }}在本项目周期内的项目投入</b><small>周期：{{ row.allocationPlan.periodStartDate }} 至 {{ row.allocationPlan.periodEndDate || '不限期' }}；当前调整生效日：{{ row.allocationPlan.effectiveDate }}</small></div><el-button size="small" @click="loadStaffAllocationPlan(row,true)">刷新</el-button></div>
                        ...
                        <template v-if="row.allocationPlan.allocations?.length">
                          ...
                          <el-input v-model="row.allocationPlan.reason" type="textarea" :rows="2" maxlength="500" show-word-limit placeholder="请填写本次跨项目投入调整原因"/>
              ...
              <el-table-column v-if="isNewTemplate" label="预计人员成本" min-width="180"><template #default="{row}"><b>{{ staffBudgetStatus(row)?.amount==null ? (budgetLoading?'计算中…':'待完善计划') : money(staffBudgetStatus(row).amount,staffBudgetStatus(row).currency||form.baseCurrency) }}</b><small>按投入比例及预算期间计算</small></template></el-table-column>
          ...
          <el-col v-else :span="24"><el-alert class="phase-alert" title="人员配置由子项目负责人完成" description="保存项目框架后，子负责人会选择本人及其他成员，并为每个人设置投入比例。" type="info" :closable="false" show-icon /></el-col>
    ...
    <el-drawer v-model="detailVisible" size="min(720px,94vw)" append-to-body>
      ...
      <div class="drawer-actions"><el-button v-if="detail.canEdit" type="primary" @click="openForm(detail)">修改</el-button><el-button v-if="detail.canEdit&&canLaunchProposal(detail)" type="success" @click="submitRow(detail)">启动项目</el-button><el-button v-if="detail.status==='APPROVED'&&detail.createdProjectId" @click="openProject(detail)">查看正式项目</el-button></div>
    </el-drawer>
  </div>
</template>
...
import { listProjectProposals, listProposalDirectory, getProjectProposal, getProjectProposalOptions, getProjectProposalStaffOptions, getProjectProposalStaffAllocationPreview, estimateProjectProposalBudget, addProjectProposal, updateProjectProposal, submitProjectProposal, withdrawProjectProposal } from '@/api/business/proposal'
...
async function loadStaffAllocationPlan(row,force=false){
  ...
  try{
    ...
    if(Number(row.userId)!==Number(userId)||shortDate(row.planStartDate)!==effectiveDate||shortDate(row.planEndDate)!==(periodEndDate||''))return
...
function lineDateIssue(value,afterStartMonth=false){
  if(!value)return ''
  ...
  if(afterStartMonth&&form.value.planStartDate&&date.slice(0,7)<=form.value.planStartDate.slice(0,7))return '完成月份须晚于项目开始月份'
  return ''
}
...
function validateDetailLines(){
  ...
  for(const [name,rows,field,afterStart] of [['收入测算',form.value.revenueLines,'expectedDate',true],['支出计划',form.value.expenseLines,'occurDate',true],['量化目标',showTargetLines.value?form.value.targetLines:[],'dueDate',false]])for(const [index,row] of (rows||[]).entries())if(lineDateIssue(row[field],afterStart))return name+'第 '+(index+1)+' 行：'+lineDateIssue(row[field],afterStart)
