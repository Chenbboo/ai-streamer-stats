    ...
    <el-dialog v-model="formVisible" :title="form.proposalId ? '修改立项申请' : '发起立项申请'" width="min(900px,96vw)" append-to-body>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="112px">
        <el-row :gutter="16">
          ...
          <el-col :span="16"><el-form-item label="计划周期" required><div class="plan-period-line"><el-date-picker v-model="form.planStartDate" type="date" value-format="YYYY-MM-DD" placeholder="开始日期" style="width:100%" /><span>至</span><el-date-picker v-model="form.planEndDate" type="date" value-format="YYYY-MM-DD" :disabled="openEnded" :disabled-date="disablePlanEndDate" :placeholder="openEnded ? '不限期' : '结束日期'" style="width:100%" /><el-checkbox v-model="openEnded" @change="handleOpenEndedChange">不限期</el-checkbox></div></el-form-item></el-col>
...
const freshForm=()=>({projectName:'',sponsorOwnerUserId:null,companyDeptId:null,projectType:'GENERAL',accountingMode:'PROFIT',managementMode:'STANDARD',closeMethod:'DIRECT',managementReason:'',acceptanceCriteria:'',objective:'',applicationReason:'',planStartDate:null,planEndDate:null,priority:'MEDIUM',baseCurrency:'CNY',budgetLimit:null,noBudget:'0'})
...
function handleOpenEndedChange(value){if(value)form.value.planEndDate=null}
