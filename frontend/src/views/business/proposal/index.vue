    ...
    <el-dialog v-model="formVisible" :title="form.proposalId ? '修改立项申请' : '新建立项申请'" width="min(1180px,97vw)" append-to-body>
      <el-form ref="formRef" class="proposal-form" :model="form" :rules="rules" :validate-on-rule-change="false" label-width="112px" require-asterisk-position="left" scroll-to-error :scroll-into-view-options="{block:'center',behavior:'smooth'}">
        ...
        <el-row :gutter="16">
          ...
          <el-col :span="24"><div class="plan-section"><div class="plan-section-head"><div><h3 :class="{'required-section':requiredPlanSections.staffing}">人员计划</h3><p>{{ isNewTemplate ? '这里只确定人员及参与时间。跟随项目会自动继承项目日期；不限期从指定日期开始。人员预算按参与期间的完整工作日计算，人员成本按工作日自动累计。' : '历史申请保留原成本快照，保存时由后台按原规则核对；人员选择不展示其他人的内部费率。' }}</p></div><el-button size="small" type="primary" plain @click="addStaffing">增加人员</el-button></div>
          ...
          <el-col :span="24"><div class="plan-section budget-plan-section">
            ...
            <el-row :gutter="24" class="budget-section-grid">
              ...
              <el-col v-if="isNewTemplate && form.staffingLines?.length" :span="24"><el-alert title="负责人默认纳入预算；预算与实际均按计费成员的参与工作日计算，已核算日期保留原依据。" type="info" :closable="false" show-icon /></el-col>
