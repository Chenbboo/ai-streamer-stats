<template>
  <div class="proposal-page">
    ...
    <el-dialog v-model="formVisible" :title="form.proposalId ? '修改立项申请' : '发起立项申请'" width="760px" append-to-body>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="112px">
        <el-row :gutter="16">
          ...
          <el-col :span="8"><el-form-item label="管理模式"><el-select v-model="form.managementMode" style="width:100%"><el-option v-for="(label,value) in managementLabel" :key="value" :label="label" :value="value" /></el-select></el-form-item></el-col>
          ...
          <el-col :span="24"><el-form-item label="立项理由" prop="applicationReason"><el-input v-model="form.applicationReason" type="textarea" :rows="3" maxlength="2000" show-word-limit placeholder="说明为什么需要公司投入资源" /></el-form-item></el-col>
    ...
    <el-drawer v-model="detailVisible" size="min(720px,94vw)" append-to-body>
      ...
      <div class="detail-grid"><div><span>申请人/负责人</span><b>{{ detail.applicantName }}</b></div><div><span>审批/归属老板</span><b>{{ detail.sponsorOwnerName }}</b></div><div><span>归属公司</span><b>{{ detail.companyName }}</b></div><div><span>状态</span><b>{{ statusLabel[detail.status] || detail.status }}</b></div><div><span>计划周期</span><b>{{ detail.planStartDate }} 至 {{ detail.planEndDate }}</b></div><div><span>预算</span><b>{{ detail.noBudget==='1'?'不设置预算':money(detail.budgetLimit,detail.baseCurrency) }}</b></div></div>
      <section class="detail-section"><h3>项目目标</h3><p>{{ detail.objective }}</p></section><section class="detail-section"><h3>立项理由</h3><p>{{ detail.applicationReason }}</p></section><section v-if="detail.reviewComment" class="detail-section review-comment"><h3>审批/撤回意见</h3><p>{{ detail.reviewComment }}</p></section>
...
const managementLabel={SIMPLE:'精简',STANDARD:'标准',DELIVERY:'交付'}
...
const freshForm=()=>({projectName:'',sponsorOwnerUserId:null,companyDeptId:null,projectType:'GENERAL',accountingMode:'PROFIT',managementMode:'SIMPLE',objective:'',applicationReason:'',priority:'MEDIUM',baseCurrency:'CNY',budgetLimit:null,noBudget:'0'})
