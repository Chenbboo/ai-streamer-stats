<template>
  <div class="proposal-page">
    ...
    <el-drawer v-model="detailVisible" size="min(720px,94vw)" append-to-body>
      ...
      <section v-if="detail.budget?.monthlyForecasts?.length" class="detail-section"><h3>{{ $tr("按月预计收支与盈利") }}</h3><el-table :data="detail.budget.monthlyForecasts" size="small" max-height="420">
        <el-table-column prop="month" :label="$tr(&quot;月份&quot;)" width="95"/>
        <el-table-column :label="$tr(&quot;收入&quot;)" min-width="125"><template #default="{row}">{{ money(row.revenueAmount,detail.baseCurrency) }}</template></el-table-column>
        <el-table-column :label="$tr(&quot;业务支出&quot;)" min-width="125"><template #default="{row}">{{ money(row.plannedBusinessAmount,detail.baseCurrency) }}</template></el-table-column>
        <el-table-column :label="$tr(&quot;人员成本&quot;)" min-width="125"><template #default="{row}">{{ money(row.personnelAmount,detail.baseCurrency) }}</template></el-table-column>
        <el-table-column :label="$tr(&quot;总成本&quot;)" min-width="125"><template #default="{row}">{{ money(row.plannedTotalCost,detail.baseCurrency) }}</template></el-table-column>
        <el-table-column :label="$tr(&quot;盈利&quot;)" min-width="125"><template #default="{row}">{{ money(row.profit,detail.baseCurrency) }}</template></el-table-column>
