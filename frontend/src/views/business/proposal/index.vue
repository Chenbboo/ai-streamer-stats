    ...
    <el-dialog v-model="formVisible" :title="form.proposalId ? '修改立项申请' : '新建立项申请'" width="min(1180px,97vw)" :before-close="beforeFormClose" append-to-body>
      <el-form ref="formRef" class="proposal-form" :disabled="saving" :model="form" :rules="rules" :validate-on-rule-change="false" label-width="112px" require-asterisk-position="left" scroll-to-error :scroll-into-view-options="{block:'center',behavior:'smooth'}">
        ...
        <el-row :gutter="16">
          ...
          <el-col :span="24"><div class="plan-section"><div class="plan-section-head"><div><h3 :class="{'required-section':requiredPlanSections.staffing}">人员计划</h3></div><el-button size="small" type="primary" plain @click="addStaffing">增加人员</el-button></div>
            ...
            </el-table><details v-if="isNewTemplate" class="inline-help"><summary>人员成本计算说明</summary><p>月成本取项目开始日的有效费率，标准折算日成本仅供参考。整月参与按月成本计算，不足整月按当月参与工作日占比分摊，月中调价分段计算。实际成本另按项目投入比例分配。</p></details></div></el-col>
...
form.value.proposalId=res.data.proposalId;form.value.version=res.data.version;formBaseline=formSnapshot();if(launch){try{const started=await submitProjectProposal(res.data.proposalId);ElMessage.success('项目已启动');formVisible.value=false;detailVisible.value=false;await refreshAll();if(started.data?.createdProjectId)openProject(started.data)}catch(error){await showPlanError(error?.message||'启动未完成，草稿已保存');await refreshAll()}}else{formVisible.value=false;ElMessage.success('草稿已保存');await refreshAll();await openDetail(res.data)}return true}catch(error){return false}finally{saving.value=false}}
