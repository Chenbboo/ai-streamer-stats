<template>
  <div class="proposal-page">
    ...
    <el-dialog v-model="formVisible" :title="form.proposalId ? '修改立项申请' : '新建立项申请'" width="min(1180px,97vw)" append-to-body>
      <el-form ref="formRef" class="proposal-form" :model="form" :rules="rules" :validate-on-rule-change="false" label-width="112px" require-asterisk-position="left" scroll-to-error :scroll-into-view-options="{block:'center',behavior:'smooth'}">
        <p class="required-hint"><span>*</span> 为启动前必填项；填写中可保存草稿。</p>
        ...
        <el-row :gutter="16">
          ...
          <el-col :span="24"><el-form-item label="目标模式" prop="goalMode" required><el-radio-group v-model="form.goalMode"><el-radio value="TOTAL">按项目总目标跟踪</el-radio><el-radio value="NO_TOTAL">持续经营，按每日目标与成果跟踪</el-radio></el-radio-group></el-form-item></el-col><el-col :span="24"><el-form-item label="项目目标" prop="objective"><el-input v-model="form.objective" type="textarea" :rows="3" maxlength="1000" show-word-limit placeholder="说明项目要解决的问题和整体目标，具体验收标准填写在项目验收目标中" /></el-form-item></el-col>
          ...
          <el-col :span="24"><div class="plan-section"><div class="plan-section-head"><div><h3 :class="{'required-section':requiredPlanSections.staffing}">人员计划</h3><p>{{ isNewTemplate ? '这里只确定人员及参与时间。跟随项目会自动继承项目日期；不限期从指定日期开始。月成本制整月参与直接使用月度用人成本，不足整月按当月参与工作日占比分摊。' : '历史申请保留原成本快照，保存时由后台按原规则核对；人员选择不展示其他人的内部费率。' }}</p></div><el-button size="small" type="primary" plain @click="addStaffing">增加人员</el-button></div>
            <el-table :data="form.staffingLines" size="small" empty-text="暂无人员计划，可在项目启动后安排">
              ...
              <el-table-column label="标准折算日成本" min-width="155"><template #default="{row}">{{ staffRateDisplay(row,'dailyCost') }}</template></el-table-column>
              <template v-if="isNewTemplate">
                ...
                <el-table-column label="参与时间" label-class-name="required-column" min-width="320"><template #default="{row}"><span v-if="row.participationMode==='FOLLOW_PROJECT'">{{ planPeriod(form) }}</span><div v-else class="staff-period"><el-date-picker v-model="row.planStartDate" type="date" value-format="YYYY-MM-DD" :disabled-date="disableStaffStartDate" placeholder="开始日期" @change="syncParticipation(row)" /><template v-if="row.participationMode==='CUSTOM'"><span>至</span><el-date-picker v-model="row.planEndDate" type="date" value-format="YYYY-MM-DD" :disabled-date="date=>disableStaffEndDate(date,row)" placeholder="结束日期" @change="syncParticipation(row)" /></template><span v-else>至 不限期</span></div></template></el-table-column>
              </template>
            ...
            </el-table><p class="field-help">月成本取项目开始日的有效费率；标准折算日成本仅供参考。月成本制整月按月成本，不足整月按当月工作日占比分摊，月中调价分段计算。请按真实薪酬资料补齐提示中的费率缺口。</p></div></el-col>

          <el-col v-if="requiredPlanSections.targets" :span="24"><ProposalPlanDetails kind="targets" :form="form" :required-plan-sections="requiredPlanSections" :target-type-label="targetTypeLabel" :target-unit-options="targetUnitOptions" :revenue-type-label="revenueTypeLabel" :disable-staff-start-date="disableStaffStartDate" :line-date-issue="lineDateIssue" @add-target="addTarget" @add-revenue="addRevenue" @change-target-type="changeTargetType" /></el-col>
          <el-col v-if="requiredPlanSections.revenue" :span="24"><ProposalPlanDetails kind="revenue" :form="form" :required-plan-sections="requiredPlanSections" :target-type-label="targetTypeLabel" :target-unit-options="targetUnitOptions" :revenue-type-label="revenueTypeLabel" :disable-staff-start-date="disableStaffStartDate" :line-date-issue="lineDateIssue" @add-target="addTarget" @add-revenue="addRevenue" @change-target-type="changeTargetType" /></el-col>

          <el-col :span="24"><div class="plan-section budget-plan-section">
            <div class="plan-section-head"><div><h3>预算设置</h3><p>设置预算控制方式和预算期间；系统结合人员计划与支出计划自动生成本期测算。</p></div><el-tag type="info" effect="plain">计划预算</el-tag></div>
    ...
    <el-drawer v-model="detailVisible" size="min(720px,94vw)" append-to-body>
      ...
      <section v-if="!detailIsIncompleteDraft" class="detail-section"><h3>经营测算</h3><div class="finance-summary compact"><div><span>预计收入</span><b>{{ money(detail.estimatedRevenue,detail.baseCurrency) }}</b></div><div><span>外部支出</span><b>{{ money(detail.estimatedExternalCost,detail.baseCurrency) }}</b></div><div><span>人员成本</span><b>{{ money(detail.estimatedPersonnelCost,detail.baseCurrency) }}</b></div><div><span>预计总成本</span><b>{{ money(detail.estimatedTotalCost,detail.baseCurrency) }}</b></div><div><span>预计利润</span><b>{{ money(detail.expectedProfit,detail.baseCurrency) }}</b></div><div><span>计划人数</span><b>{{ detail.plannedHeadcount || 0 }} 人</b></div></div></section>
      <section v-if="detail.budget?.steadyMonth" class="detail-section"><h3>下月预计成本</h3><p>{{ detail.budget.steadyMonth.startDate }} 至 {{ detail.budget.steadyMonth.endDate }}，按立项人员计划和当前有效费率测算。月成本制整月按月度用人成本计入。</p><div class="finance-summary compact"><div><span>人员成本</span><b>{{ money(detail.budget.steadyMonth.personnelAmount,detail.baseCurrency) }}</b></div><div><span>外部支出</span><b>{{ money(detail.budget.steadyMonth.plannedBusinessAmount,detail.baseCurrency) }}</b></div><div><span>预计总成本</span><b>{{ money(detail.budget.steadyMonth.plannedTotalCost,detail.baseCurrency) }}</b></div></div><el-alert v-if="detail.budget.steadyMonth.status==='PENDING'" title="下月测算待完善" :description="detail.budget.steadyMonth.issues?.join('；')" type="warning" :closable="false" /></section>
...
const accountingOptions={PROFIT:{label:'盈利型',example:'电商销售',description:'看收入、成本和赚了多少'},COST:{label:'成本型',example:'办公装修',description:'看花了多少，有没有超预算'},VALUE:{label:'价值型',example:'员工培训',description:'看投入多少，目标有没有完成'},HYBRID:{label:'混合型',example:'新品推广',description:'既看赚了多少，也看目标完成情况'}}
...
// Required sections follow accounting mode; optional details remain available below.
const requiredPlanSections=computed(()=>({
  staffing:!isNewTemplate.value,
  targets:isNewTemplate.value||['VALUE','HYBRID'].includes(form.value.accountingMode)||form.value.goalMode!=='NO_TOTAL',
  revenue:['PROFIT','HYBRID'].includes(form.value.accountingMode)
}))
const showTargetLines=computed(()=>isNewTemplate.value||form.value.goalMode!=='NO_TOTAL'||requiredPlanSections.value.targets)
...
const supplementaryPanels=ref([])
function expandExistingSupplementary(){
  const panels=[]
  if(form.value.expenseLines?.length)panels.push('expenses')
  if(!requiredPlanSections.value.revenue&&form.value.revenueLines?.length)panels.push('revenue')
  if(!requiredPlanSections.value.targets&&form.value.targetLines?.length)panels.push('targets')
  if(isNewTemplate.value&&form.value.riskSummary?.trim())panels.push('risk')
  supplementaryPanels.value=panels
}
watch(()=>form.value.accountingMode,()=>{expandExistingSupplementary();nextTick(()=>formRef.value?.clearValidate())})
async function showPlanError(message){
  ...
  if(section&&!supplementaryPanels.value.includes(section))supplementaryPanels.value.push(section)
  await nextTick()
...
function validateDetailLines(){
  ...
  for(const [name,rows,field] of [['收入测算',form.value.revenueLines,'expectedDate'],['支出计划',form.value.expenseLines,'occurDate'],['量化目标',showTargetLines.value?form.value.targetLines:[],'dueDate']])for(const [index,row] of (rows||[]).entries())if(lineDateIssue(row[field]))return name+'第 '+(index+1)+' 行：'+lineDateIssue(row[field])
...
const staffing=form.value.staffingLines||[];const payload={...form.value,...budgetPolicyPayload(),saveAsDraft:!launch&&isNewTemplate.value,targetLines:showTargetLines.value?form.value.targetLines:[],staffingLines:staffing.map(item=>({...item,inputUnit:'PERCENTAGE',inputQuantity:100,unitPolicyId:item.unitPolicyId||defaultUnitPolicy()})),estimatedBonusCost:0,planEndDate:openEnded.value?null:form.value.planEndDate,budget:{...form.value.budget,cycle:openEnded.value?form.value.budget.cycle:'PROJECT',projectOpenEnded:openEnded.value}};const res=payload.proposalId?await updateProjectProposal(payload):await addProjectProposal(payload);form.value.proposalId=res.data.proposalId;form.value.version=res.data.version;if(launch){try{const started=await submitProjectProposal(res.data.proposalId);ElMessage.success('项目已启动');formVisible.value=false;detailVisible.value=false;await refreshAll();if(started.data?.createdProjectId)openProject(started.data)}catch(error){await showPlanError(error?.message||'启动未完成，草稿已保存');await refreshAll()}}else{formVisible.value=false;ElMessage.success('草稿已保存，可继续完善后启动');await refreshAll();await openDetail(res.data)}}finally{saving.value=false}}
...
@media(max-width:760px){.plan-period-line{grid-template-columns:1fr}.plan-period-line>span{display:none}.budget-total-card{align-items:flex-start;flex-direction:column}.budget-total-card strong{text-align:left}.budget-breakdown,.budget-meta{grid-template-columns:1fr}.budget-meta>div:nth-child(even){padding-left:0;border-left:0;border-top:1px solid #e8edf2}.budget-meta .budget-reason{grid-column:auto}}
