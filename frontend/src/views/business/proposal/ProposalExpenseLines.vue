<template>
  <div class="plan-section form-section">
    <div class="plan-section-head">
      <div><h3><span class="section-index">{{ sectionNumber }}</span>支出计划<span class="optional-label">（选填）</span></h3><p>{{ description }}</p></div>
    ...
    <el-table :data="form.expenseLines" size="small" empty-text="暂无外部支出">
      ...
      <el-table-column label="金额" :label-class-name="form.expenseLines?.length ? 'required-column' : ''" width="165"><template #default="{row}"><el-input-number v-model="row.amount" :min="0" :precision="2" controls-position="right"/></template></el-table-column>
