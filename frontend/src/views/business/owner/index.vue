    ...
    <header class="owner-hero">
      ...
      <div class="hero-actions">
        ...
        <el-select v-model="selectedProjectId" filterable placeholder="选择负责项目" @change="switchProject">
          <el-option v-for="item in projects" :key="item.projectId" :label="`${item.projectName} · ${item.sponsorOwnerName || item.initiatorName || '未标注老板'} · ${statusLabel[item.status] || item.status}`" :value="item.projectId" />
        </el-select>
    ...
    <template v-if="project">
      ...
      <section v-if="allocationAlerts.length" class="panel allocation-alert-panel">
        ...
        <div v-for="item in allocationAlerts" :key="item.projectId" class="allocation-alert-row">
          <span><b>{{ item.projectName }}</b><small>{{ item.projectNo || '暂无项目编号' }} · {{ statusLabel[item.status] || item.status }}</small></span>
      ...
      <section class="workspace-grid">
        ...
        <aside class="side-column">
          <article class="panel project-summary">
            ...
            <dl><div><dt>归属老板</dt><dd>{{ project.sponsorOwnerName || project.initiatorName }}</dd></div><div><dt>归属公司</dt><dd>{{ project.companyName || '待设置' }}</dd></div><div><dt>计划周期</dt><dd>{{ project.planStartDate || '—' }} 至 {{ project.planEndDate || '—' }}</dd></div><div><dt>项目进度</dt><dd>{{ projectProgress }}%</dd></div></dl>
          </article>
