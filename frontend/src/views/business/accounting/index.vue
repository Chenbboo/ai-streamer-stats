<template>
  <div class="app-container accounting-page">
    <header class="hero"><div><span>DAILY BUSINESS LEDGER</span><h1>项目核算与收支</h1><p>按公司、项目和日期记录经营事实，确认后自动生成项目盈亏快照。</p></div><div v-if="canAddFact" class="hero-actions"><ProfitTaxSettings @changed="load" /><el-button type="success" icon="Plus" @click="openEntry('REVENUE')">录入收入</el-button><el-button type="primary" icon="Plus" @click="openEntry('COST')">录入支出</el-button></div></header>
    <el-card shadow="never" class="filter-card"><el-form :inline="true" :model="query"><el-form-item><el-select v-model="query.companyDeptId" clearable placeholder="全部授权公司" style="width:190px"><el-option v-for="c in data.companies" :key="c.companyDeptId" :label="c.companyName" :value="c.companyDeptId" /></el-select></el-form-item><el-form-item><el-select v-model="query.projectId" clearable filterable placeholder="全部本人项目" style="width:220px"><el-option v-for="p in filteredProjects" :key="p.projectId" :label="`${p.projectName} · ${p.initiatorName}`" :value="p.projectId" /></el-select></el-form-item><el-form-item><el-date-picker v-model="selectedMonth" type="month" value-format="YYYY-MM" format="YYYY年MM月" :clearable="false" @change="monthChanged" /></el-form-item><el-form-item><el-button type="primary" @click="load">查询</el-button><el-button @click="reset">本月</el-button></el-form-item></el-form></el-card>
    <BusinessSettlementPanel v-if="query.projectId" :project="selectedProject" @closed="load" />
    <el-alert v-if="data.taxUnconfiguredCount" title="部分项目尚未设置公司税率，税额暂按0计算；请由老板设置税率。" type="warning" :closable="false" show-icon /><section v-if="hasIncompleteCosts" class="pricing-notice">
        ...
        <el-table-column type="expand">
          <template #default="{ row }">
            <div class="department-projects">
              <p>{{ row.departmentName }}项目明细</p>
              <el-table :data="row.projects" size="small" row-key="projectId">
                <el-table-column label="项目" min-width="180"><template #default="{row:projectRow}"><b>{{ projectRow.projectName }}</b><small>{{ projectRow.projectNo }}</small></template></el-table-column>
                <el-table-column label="收入" align="right"><template #default="{row:projectRow}">{{ money(projectRow.revenueAmount) }}</template></el-table-column>
                <el-table-column label="业务成本" align="right"><template #default="{row:projectRow}">{{ money(projectRow.businessCost) }}</template></el-table-column>
                <el-table-column label="人员成本" align="right"><template #default="{row:projectRow}">{{ money(projectRow.personnelCost) }}</template></el-table-column>
                <el-table-column label="项目奖金" align="right"><template #default="{row:projectRow}">{{ money(projectRow.bonusCost) }}</template></el-table-column>
                <el-table-column label="公共费用" align="right"><template #default="{row:projectRow}">{{ money(projectRow.publicCost) }}</template></el-table-column>
                <el-table-column label="税前结果" align="right"><template #default="{row:projectRow}"><b :class="amountTone(projectRow.pretaxProfit)">{{ signed(projectRow.pretaxProfit) }}</b></template></el-table-column>
              </el-table>
            </div>
          </template>
        </el-table-column>
        <el-table-column label="部门" min-width="190"><template #default="{row}"><b>{{ row.departmentName }}</b><small>{{ row.companyName }} · {{ row.projects.length }} 个项目</small></template></el-table-column>
        <el-table-column prop="currency" label="币种" width="75" />
        <el-table-column label="确认收入" align="right"><template #default="{row}">{{ money(row.revenueAmount) }}</template></el-table-column>
        <el-table-column label="总成本" align="right"><template #default="{row}">{{ money(row.totalCost) }}</template></el-table-column>
        <el-table-column label="税前结果" align="right"><template #default="{row}"><b :class="amountTone(row.pretaxProfit)">{{ signed(row.pretaxProfit) }}</b></template></el-table-column>
        <el-table-column label="税额" align="right"><template #default="{row}">{{ money(row.taxAmount) }}</template></el-table-column>
        <el-table-column label="税后结果" align="right"><template #default="{row}"><b :class="amountTone(row.afterTaxProfit)">{{ signed(row.afterTaxProfit) }}</b></template></el-table-column>
      </el-table>
    </el-card>
    <el-card shadow="never" class="section-card"><div class="section-head"><div><h2>收支流水</h2><p>保存收支后直接入账并更新项目日结果；普通已入账流水可冲销；公共费用请在原月账登记调整。</p></div></div><el-table  :data="pageRows(data.facts, 'facts')"><el-table-column prop="bizDate" label="业务日期" width="110"/><el-table-column prop="recordedTime" label="录入时间" width="180"/><el-table-column label="操作人名称" min-width="110"><template #default="{row}">{{ row.operatorName||'—' }}</template></el-table-column><el-table-column label="操作人账号" min-width="130"><template #default="{row}">{{ row.operatorAccount||row.createBy||'—' }}</template></el-table-column><el-table-column label="项目" min-width="170"><template #default="{row}">{{ row.projectName }}<small>{{ row.companyName }}</small></template></el-table-column><el-table-column prop="categoryName" label="类别"/><el-table-column prop="description" label="说明" min-width="180"/><el-table-column label="金额/数值" align="right"><template #default="{row}"><b>{{ row.factKind==='VALUE'?`${row.quantity} ${row.unit||''}`:`${signed(row.amount)} ${row.currency||''}` }}</b></template></el-table-column><el-table-column label="状态" min-width="135"><template #default="{row}"><el-tag :type="statusTone[row.status]">{{ statusLabel[row.status] || row.status }}</el-tag><small v-if="row.status==='RETURNED'" class="return-reason">原因：{{ row.returnReason }}</small></template></el-table-column><el-table-column label="操作" min-width="210" fixed="right"><template #default="{row}"><el-button v-if="isBoss||row.sourceDomain==='HR_INCENTIVE'||!['DRAFT','RETURNED'].includes(row.status)" link type="primary" @click="openFact(row,true)">查看详情</el-button><el-button v-else-if="row.sourceDomain!=='HR_INCENTIVE' && isFactAccountingOpen(row) && ['DRAFT','RETURNED'].includes(row.status)" link @click="openFact(row)">{{ row.status==='RETURNED'?'修改并重新提交':'编辑' }}</el-button><el-button v-if="canReviewFact(row)&&isFactAccountingOpen(row)&&row.status==='DRAFT'" link type="success" @click="confirmFact(row)">确认入账</el-button><el-button v-if="canReviewFact(row)&&isFactAccountingOpen(row)&&row.status==='DRAFT'" link type="warning" @click="returnFact(row)">退回修改</el-button><el-button v-if="canReviewFact(row)&&isFactAccountingOpen(row)&&row.status==='CONFIRMED'" link type="danger" @click="reverseFact(row)">冲销</el-button></template></el-table-column></el-table><el-pagination class="table-pagination" v-model:current-page="tablePages.facts.page" v-model:page-size="tablePages.facts.size" :page-sizes="[5, 10, 20, 50]" :pager-count="5" :total="data.facts.length" layout="total, sizes, prev, pager, next" background /></el-card>
    ...
    <el-dialog v-model="factDialog" :title="factDialogTitle" width="min(680px,94vw)" append-to-body>
      ...
      <el-alert v-if="factForm.sourceDomain==='HR_INCENTIVE'" title="此成本来自已核准奖励单，不能修改金额或来源；退回后请在奖金激励中续办。成本入账不代表已经发放。" :closable="false"/>
      <el-alert v-if="factForm.status==='RETURNED'" class="return-alert" :title="`退回原因：${factForm.returnReason||'未填写'}`" :description="`${factForm.returnedUserName||'老板'} 于 ${factForm.returnedTime||'—'} 退回，修改保存后直接入账。`" type="warning" :closable="false" show-icon/>
      <el-form :model="factForm" :disabled="factReadOnly" label-width="92px" class="dialog-form">
        <el-form-item label="归属公司" required>
          <el-select v-model="factForm.companyDeptId" :disabled="factReadOnly||!!factForm.factId" filterable clearable placeholder="输入公司名称搜索" no-data-text="暂无可选公司" style="width:100%" @change="companyChanged">
        ...
        <el-form-item label="归属项目" required>
          <el-select v-model="factForm.projectId" :disabled="factReadOnly||!!factForm.factId||!factForm.companyDeptId" filterable clearable placeholder="输入项目名称搜索" :no-data-text="factForm.companyDeptId?'该公司暂无可填报项目':'请先选择归属公司'" style="width:100%" @change="projectChanged">
            <el-option v-for="p in factProjectOptions" :key="p.projectId" :label="`${p.projectName} · ${p.companyName||'未设置公司'}`" :value="p.projectId" />
          </el-select>
        </el-form-item>
        <el-form-item label="业务日期" required><el-date-picker v-model="factForm.bizDate" :disabled-date="disabledFactDate" type="date" value-format="YYYY-MM-DD" style="width:100%" /></el-form-item>
        <el-form-item v-if="entryKind" :label="entryKind==='REVENUE'?'收入类别':'支出类别'" required><el-select v-model="factForm.categoryId" :placeholder="entryKind==='REVENUE'?'请选择收入类别':'请选择支出类别'" style="width:100%"><el-option v-for="item in entryKind==='REVENUE'?revenueCategories:expenseCategories" :key="item.categoryId" :label="item.categoryName" :value="item.categoryId" /></el-select></el-form-item>
        <el-form-item v-else-if="!entryKind" label="收支类别" required><el-select v-model="factForm.categoryId" style="width:100%"><el-option-group v-for="group in categoryGroups" :key="group.kind" :label="group.label"><el-option v-for="c in group.items" :key="c.categoryId" :label="c.categoryName" :value="c.categoryId" /></el-option-group></el-select></el-form-item>
        <template v-if="!entryKind&&selectedCategory?.factKind==='VALUE'"><el-form-item label="成果数值" required><el-input-number v-model="factForm.quantity" :precision="4" style="width:100%" /></el-form-item><el-form-item label="单位"><el-input v-model="factForm.unit" /></el-form-item></template>
        <template v-else>
          <el-form-item :label="entryKind==='REVENUE'?'收入金额':entryKind==='COST'?'支出金额':'金额'" required><el-input-number v-model="factForm.amount" :min="entryKind==='COST'?0.01:0" :precision="entryKind?2:4" style="width:100%" /></el-form-item>
          <el-form-item label="币种" :required="!!entryKind"><el-select v-if="entryKind" v-model="factForm.currency" placeholder="请选择币种" style="width:100%"><el-option v-for="currency in entryCurrencies" :key="currency" :label="currency" :value="currency" /></el-select><el-input v-else v-model="factForm.currency" maxlength="3" /></el-form-item>
        </template>
        <el-form-item :label="entryKind==='REVENUE'?'收入说明':entryKind==='COST'?'花费用途':'收支说明'" required><el-input v-model="factForm.description" type="textarea" :rows="3" maxlength="500" show-word-limit :placeholder="entryKind==='REVENUE'?'请说明收入来源或对应业务':entryKind==='COST'?'例如：广告投流、采购或物流':''" /></el-form-item>
        <el-form-item :label="entryKind==='REVENUE'?'付款单位':entryKind==='COST'?'收款单位':'对方单位'"><el-input v-model="factForm.counterparty" maxlength="200" /></el-form-item>
        <el-form-item label="凭证附件"><business-file-upload v-if="factReadOnly" :model-value="factForm.attachmentUrls" :project-id="factForm.projectId" disabled :drag="false" :is-show-tip="false"/><business-file-upload v-else v-model="factForm.attachmentUrls" :project-id="factForm.projectId" /></el-form-item>
        <el-form-item label="备注"><el-input v-model="factForm.remark" type="textarea" :rows="2" maxlength="500" show-word-limit /></el-form-item>
      </el-form>
  ...
  <el-empty v-else description="本日没有已计价人员成本分项；请查看人员成本核算中的待完善事项。" :image-size="70" />
...
const departmentGroups=computed(()=>{
  ...
  const ensureProject=source=>{
    ...
    if(!departments.has(key))departments.set(key,{key,companyDeptId,companyName:source.companyName||'未设置公司',departmentId:source.departmentId,departmentName:source.departmentName||'未设置部门',currency,projects:new Map()})
    const department=departments.get(key),projectKey=String(source.projectId)
    if(!department.projects.has(projectKey))department.projects.set(projectKey,{projectId:source.projectId,projectName:source.projectName||'未命名项目',projectNo:source.projectNo||'',revenueAmount:0,businessCost:0,personnelCost:0,bonusCost:0,publicCost:0,adjustmentAmount:0,pretaxProfit:0,taxAmount:0,afterTaxProfit:0})
...
const pricingNotice=computed(()=>Number(data.pendingCostCount)>0
  ? `当前查询范围有 ${data.pendingCostCount} 条成员工作日成本待完善；下列金额为已核算部分，不能视为完整成本或最终利润。补充有效成本或工作日历后刷新。`
  : '当前核算日有成员工作日缺少有效成本或日历，人员成本和经营结果尚不完整。')
...
const categoryGroups=computed(()=>['REVENUE','COST','ADJUSTMENT','VALUE'].map(kind=>({kind,label:{REVENUE:'收入',COST:'成本费用',ADJUSTMENT:'核算调整',VALUE:'非金额成果'}[kind],items:data.categories.filter(c=>c.factKind===kind&&(factReadOnly.value||!['COMPANY_PUBLIC_COST','PROJECT_MANAGEMENT_FEE'].includes(c.categoryCode)))})).filter(g=>g.items.length))
const factDialogTitle=computed(()=>factReadOnly.value?'收支流水详情':factForm.factId?'编辑收支草稿':entryKind.value==='REVENUE'?'录入收入':'录入支出')
const factAlertTitle=computed(()=>factReadOnly.value?'当前为查看详情，所有数据均为只读状态。':entryKind.value==='REVENUE'?'填写本次收入，保存后直接入账并更新项目经营结果。':entryKind.value==='COST'?'填写本次发生的金额，保存后直接入账并计入项目成本。':'保存后直接入账并更新项目日结果。')
...
const personnelDetailRows=computed(()=>(resultDetail.value?.personnelItems||[]).map(item=>{
  ...
  if(basis.costPolicyVersion==='MEMBER_DAYS_V1')return {...item,workText:`${basis.workingDays} 工作日 · 权重 ${basis.allocationPercent ?? 100}%`,sourceText:`${basis.bizDate} · 工作日历 #${basis.calendarId}`,rateText:`${money(basis.fullDailyCost ?? basis.dailyCost)} ${basis.currency} / 工作日 × ${basis.allocationPercent ?? 100}%`,explanation:'工作日数 × 当日有效日成本 × 项目投入权重，休息日不计费'}
...
function reset(){Object.assign(query,{companyDeptId:null,projectId:null});selectedMonth.value=today().slice(0,7);dates.value=monthRange(selectedMonth.value);load()}
...
async function saveFact(){if(factReadOnly.value)return;if(!canWriteFactProject.value)return ElMessage.warning('该项目核算不可写，请刷新核对状态');if(!factForm.companyDeptId||!factForm.projectId||!factForm.categoryId||!factForm.bizDate)return ElMessage.warning('请完整选择公司、项目、日期和类别');if(entryKind.value==='REVENUE'&&(factForm.amount===null||factForm.amount===undefined||Number(factForm.amount)<0))return ElMessage.warning('请填写收入金额');if(entryKind.value==='COST'&&!(Number(factForm.amount)>0))return ElMessage.warning('本次花费必须大于 0');if(!factForm.description?.trim())return ElMessage.warning(entryKind.value==='REVENUE'?'请填写收入说明':entryKind.value==='COST'?'请填写花费用途':'请填写收支说明');const wasReturned=factForm.status==='RETURNED',kind=entryKind.value,savedCompanyDeptId=factForm.companyDeptId,savedProjectId=factForm.projectId,savedBizDate=factForm.bizDate;saving.value=true;try{await saveBusinessOperatingFact({...factForm,description:factForm.description.trim(),counterparty:factForm.counterparty?.trim(),currency:(factForm.currency||factProject.value?.currency||'CNY').trim().toUpperCase()});factDialog.value=false;query.companyDeptId=savedCompanyDeptId;query.projectId=savedProjectId;selectedMonth.value=savedBizDate.slice(0,7);dates.value=monthRange(selectedMonth.value);await load();ElMessage.success(wasReturned?'已修改并入账':kind==='REVENUE'?'收入已入账':kind==='COST'?'支出已入账':'收支已入账')}finally{saving.value=false}}
...
watch(()=>[route.path,route.query.companyDeptId,route.query.projectId,route.query.month,route.query.dateFrom],async()=>{
  ...
  dates.value=monthRange(selectedMonth.value)
