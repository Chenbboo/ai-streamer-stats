<template>
  <div class="app-container accounting-page">
    ...
    <el-card shadow="never" class="section-card department-result-card">
      <div class="section-head"><div><h2>{{ $tr("部门月结果 · {0} 个部门", [departmentGroups.length]) }}</h2><p>{{ $tr("{0}，点击部门可查看该部门下所有项目的营收与成本。", [selectedMonthLabel]) }}</p></div></div>
...
const selectedMonthLabel=computed(()=>translateText("{0}年{1}月", [selectedMonth.value.slice(0,4), selectedMonth.value.slice(5,7)]))
