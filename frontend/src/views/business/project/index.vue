  ...
  <div class="app-container project-page">
    ...
    <el-card shadow="never" class="filter-card">
      <el-form :inline="true" :model="query" @submit.prevent>
        ...
        <el-form-item><el-select v-model="query.companyDeptId" clearable filterable placeholder="全部公司" style="width:200px" @change="query.mainOwnerDeptId=''" @visible-change="visible => visible && loadFilterOptions()"><el-option v-for="company in companies" :key="company.companyDeptId" :label="company.companyName" :value="company.companyDeptId" /></el-select></el-form-item>
        <el-form-item><el-select v-model="query.mainOwnerDeptId" clearable filterable placeholder="全部部门（主负责人）" style="width:220px" @visible-change="visible => visible && loadFilterOptions()"><el-option v-for="department in filterDepartments" :key="department.deptId" :label="query.companyDeptId ? department.deptName : `${department.companyName} / ${department.deptName}`" :value="department.deptId" /></el-select></el-form-item>
    ...
    <el-drawer v-model="detailVisible" size="min(920px, 96vw)" destroy-on-close @closed="detail = null">
      ...
      <template v-if="detail">
        ...
        <section v-if="showStageClosureGuard" :class="['kpi-close-guard', `is-${stageClosureState.tone}`]">
          ...
          <div class="stage-close-actions">
            <el-button v-if="stageClosureState.canRequest" type="success" @click="runTransition({key:'REQUEST_CLOSE',label:'申请结项',type:'success'})">提交{{ acceptanceReviewerLabel }}检验</el-button>
            <template v-if="stageClosureState.canReview"><el-button type="success" @click="runTransition({key:'CLOSE',label:'确认结项并冻结',type:'success'})">确认结项并冻结</el-button><el-button type="warning" plain @click="runTransition({key:'RETURN_ACTIVE',label:'退回补充'})">退回补充</el-button></template>
          </div>
        </section>
        <section v-if="showDirectClosureGuard" class="kpi-close-guard is-warning">
          <div class="kpi-close-mark">审核</div>
          <div class="kpi-close-copy">
            <div class="kpi-close-title"><span>直接结项检验</span><el-tag type="warning" effect="light">待{{ acceptanceReviewerLabel }}检验</el-tag></div>
            <b>负责人已提交直接结项申请</b>
            <p>{{ canReviewAcceptance ? '请核对项目成果和结项前置条件，通过后项目才会正式关闭。' : `正在等待${acceptanceReviewerLabel}检验；子项目负责人无权自行通过。` }}</p>
          </div>
          <div v-if="canReviewAcceptance" class="stage-close-actions"><el-button type="success" @click="runTransition({key:'CLOSE',label:'检验通过、结项并冻结',type:'success'})">检验通过、结项并冻结</el-button><el-button type="warning" plain @click="runTransition({key:'RETURN_ACTIVE',label:'退回补充'})">退回补充</el-button></div>
        </section>
        <el-tabs ref="detailTabs" v-model="activeTab" class="project-detail-tabs">
          <el-tab-pane label="项目总览" name="overview">
            ...
            <section class="cockpit-metrics" v-loading="cockpitLoading">
              ...
              <article><span>累计税额</span><b>{{ money(cockpitSummary.taxAmount) }}</b><small>按公司累计结果分配，含税额冲回</small></article><article :class="Number(cockpitSummary.afterTaxProfit)<0?'is-danger':'is-success'"><span>{{ cockpitCostIncomplete?'已核算税后盈利':'税后盈利结果' }}</span><b>{{ signedMoney(cockpitSummary.afterTaxProfit) }}</b><small>{{ cockpit.taxUnconfiguredCount?'税率未设置，暂按0%':cockpitCostIncomplete?'成本待完善':cockpitCurrency }}</small></article><article><span>经营结果天数</span><b>{{ cockpitSummary.resultCount || 0 }}</b><small>{{ cockpitDateRange }}</small></article>
            </section>
            ...
            <div class="cockpit-columns">
              <section class="cockpit-card">
                ...
                <el-alert v-if="kpiWorkspaceError" title="KPI自动数据暂时无法读取，请刷新后重试。" type="warning" :closable="false" show-icon />
          ...
          <el-tab-pane v-if="!usesActualWork" label="历史经营配置" name="operating">
            <div class="operating-grid">
              ...
              <section class="operating-card"><div class="operating-head"><div><small>当前KPI</small><strong>{{ currentKpis.length }} 项</strong></div><el-button v-if="isBoss||myRole==='OWNER'" size="small" type="primary" @click="router.push({path:'/business/kpi-bonus',query:{projectId:detail.projectId}})">{{ isDeliveryEnded(detail) ? '查看KPI结算' : '配置项目指标' }}</el-button></div><p>项目负责人设置指标目标和权重，发布方案时统一设置考核时间；老板保留修改能力。</p></section>
            </div>
          ...
          <el-tab-pane label="持续工作" name="routines">
            ...
            <el-table :data="allProjectRoutines" size="small" empty-text="尚未设置持续工作">
              <el-table-column label="工作内容" min-width="220"><template #default="{row}"><b>{{ row.routineName }}</b><small v-if="row.sourceManaged" class="subline">直播同步 · 数据日期 {{ row.sourceBizDate }}</small><small v-else-if="row.status==='VOID'" class="subline danger">已停用，历史数据保留</small></template></el-table-column>
              <el-table-column label="目标方式" min-width="170"><template #default="{row}"><b>{{ routineTargetModeLabel[row.targetMode || 'FIXED'] }}</b><small class="subline">{{ routineTargetText(row) }}</small></template></el-table-column>
              ...
              <el-table-column label="监督 / 执行区间记录" min-width="240"><template #default="{row}"><span v-if="row.sourceManaged">{{ row.supervisorName }}监督 · {{ row.startDate }} 至 {{ row.endDate || todayText() }}</span><div v-else class="period-list"><span v-for="(period,index) in workPeriods(row)" :key="period.periodId||index">第 {{ index+1 }} 段：{{ executionPeriodText(period) }}</span></div></template></el-table-column>
              <el-table-column v-if="canManage" label="操作" width="120"><template #default="{row}"><el-tag v-if="row.sourceManaged" type="info" effect="plain">自动同步</el-tag><template v-else-if="row.status==='ACTIVE'"><el-button link @click="openRoutine(row)">编辑</el-button><el-button link type="danger" @click="removeRoutine(row)">停用</el-button></template><el-button v-else link type="success" @click="enableRoutine(row)">启用</el-button></template></el-table-column>
            </el-table>
          </el-tab-pane>
          <el-tab-pane label="一次性任务" name="tasks"><div class="tab-tools"><div><b>一次性任务</b><span class="muted routine-tip">停用只结束当前执行区间，任务资料和填报记录会继续保留。</span></div><el-button v-if="canManage" size="small" type="primary" @click="openItem('task')">新增任务</el-button></div><el-table :data="allProjectTasks" size="small"><el-table-column label="任务" min-width="180"><template #default="{row}"><b>{{ row.taskName }}</b><small v-if="row.activeStatus==='VOID'" class="subline danger">已停用，历史数据保留</small></template></el-table-column><el-table-column v-if="showMilestones" label="所属里程碑" min-width="130"><template #default="{row}">{{ milestoneName(row.milestoneId) }}</template></el-table-column><el-table-column prop="assigneeName" label="负责人" width="110" /><el-table-column label="状态" width="100"><template #default="{row}">{{ row.activeStatus==='VOID'?'已停用':taskStatusLabel[row.status] }}</template></el-table-column><el-table-column label="进度" width="115"><template #default="{row}"><el-progress :percentage="row.progress || 0" :stroke-width="7" /></template></el-table-column><el-table-column label="执行区间记录" min-width="220"><template #default="{row}"><div class="period-list"><span v-for="(period,index) in workPeriods(row)" :key="period.periodId||index">第 {{ index+1 }} 段：{{ executionPeriodText(period) }}</span></div></template></el-table-column><el-table-column prop="dueDate" label="截止日期" width="115" /><el-table-column v-if="canManage" label="操作" width="120"><template #default="{row}"><template v-if="row.activeStatus!=='VOID'"><el-button link @click="openItem('task',row)">编辑</el-button><el-button link type="danger" @click="removeItem('task',row)">停用</el-button></template><el-button v-else link type="success" @click="enableTask(row)">启用</el-button></template></el-table-column></el-table></el-tab-pane>
          <el-tab-pane label="成员" name="members"><div class="tab-tools"><b>项目成员</b><el-button v-if="canManage" size="small" type="primary" @click="openItem('member')">添加成员</el-button></div><el-table :data="detail.members" size="small"><el-table-column prop="userNameSnapshot" label="姓名" /><el-table-column label="项目角色"><template #default="{row}">{{ memberRoleLabel[row.memberRole] }}</template></el-table-column><el-table-column prop="joinedDate" label="加入日期" /><el-table-column v-if="canManage" label="操作" width="145"><template #default="{row}"><el-button v-if="row.memberRole !== 'OWNER' && (row.memberRole !== 'DEPUTY' || canManageDeputies)" link type="primary" @click="openItem('member',row)">调整角色</el-button><el-button v-if="row.memberRole !== 'OWNER' && (row.memberRole !== 'DEPUTY' || canManageDeputies)" link type="danger" @click="removeItem('member',row)">移除</el-button></template></el-table-column></el-table></el-tab-pane>
      <el-tab-pane v-if="showMilestones" label="里程碑" name="milestones"><div class="tab-tools"><b>关键里程碑</b><el-button v-if="canManage" size="small" type="primary" @click="openItem('milestone')">新增里程碑</el-button></div><el-table :data="detail.milestones" size="small"><el-table-column prop="milestoneName" label="里程碑" /><el-table-column prop="planDate" label="计划日期" width="115" /><el-table-column label="状态" width="100"><template #default="{row}">{{ milestoneStatusLabel[row.status] || row.status }}</template></el-table-column><el-table-column v-if="canManage" label="操作" width="110"><template #default="{row}"><template v-if="!['REVIEWING','DONE'].includes(row.status)"><el-button link @click="openItem('milestone',row)">编辑</el-button><el-button link type="danger" @click="removeItem('milestone',row)">删除</el-button></template><el-tag v-else type="info" effect="plain">验收锁定</el-tag></template></el-table-column></el-table></el-tab-pane>
          ...
          <el-tab-pane v-if="detail.closeMethod==='RESULT_ACCEPTANCE'" label="成果验收" name="acceptance"><div class="tab-tools"><b>验收资料与{{ acceptanceReviewerLabel }}意见</b><el-button v-if="canSubmitAcceptance" size="small" type="success" :disabled="kpiClosureState.ready===false" :title="kpiClosureState.ready===false?'请先完成并确认全部KPI结算':''" @click="openAcceptanceSubmit">提交验收</el-button></div>
            ...
            <article v-for="record in detail.acceptances" :key="record.acceptanceId" class="acceptance-record">
              ...
              <div v-if="record.reviewStatus==='PENDING' && canReviewAcceptance && detail.status==='ACCEPTANCE'" class="review-actions"><el-button type="success" :disabled="kpiClosureState.ready===false" :title="kpiClosureState.ready===false?'请先完成并确认全部KPI结算':''" @click="openAcceptanceReview('APPROVED',record)">验收通过、结项并冻结</el-button><el-button type="warning" plain @click="openAcceptanceReview('RETURNED',record)">退回执行</el-button></div>
            </article>
          </el-tab-pane>
                  ...
                  <div class="event-head">
                    <div class="event-actor">
                      ...
                      <div class="event-meta">
                        <span>账号：<strong>{{ eventActorAccount(event) }}</strong></span>
                        <span v-if="eventWorkAssignee(event)">执行人：<strong>{{ eventWorkAssignee(event) }}</strong></span>
    ...
    <el-dialog v-model="routineDialog" :title="routineForm.routineId?'编辑持续工作':'新增持续工作'" width="min(620px, 94vw)" append-to-body>
      ...
      <el-form :model="routineForm" label-width="96px" class="decision-form">
        ...
        <el-form-item label="目标方式" required><el-select v-model="routineForm.targetMode" style="width:100%" @change="handleRoutineTargetModeChange"><el-option label="固定每日目标" value="FIXED"/><el-option v-if="detail?.goalMode!=='NO_TOTAL'" label="总目标自动分配到每日" value="AUTO_TOTAL"/><el-option label="客户动态日目标（负责人每日下达）" value="DAILY_DYNAMIC"/><el-option label="无量化目标（只填完成说明）" value="NONE"/></el-select></el-form-item>
        <el-row :gutter="12"><el-col :sm="12" :xs="24"><el-form-item label="目标频率" required><el-select v-model="routineForm.frequency" disabled style="width:100%"><el-option label="每日" value="DAILY" /></el-select><small class="form-tip">目标按天重复；执行区间只决定从哪天到哪天生效。</small></el-form-item></el-col><el-col :sm="12" :xs="24"><el-form-item label="负责人" required><el-select v-model="routineForm.assigneeUserId" filterable style="width:100%"><el-option v-for="m in detail?.members || []" :key="m.userId" :label="memberOptionLabel(m)" :value="m.userId" /></el-select></el-form-item></el-col></el-row>
        <el-row :gutter="12"><el-col v-if="['FIXED','AUTO_TOTAL'].includes(routineForm.targetMode)" :sm="12" :xs="24"><el-form-item :label="routineForm.targetMode==='AUTO_TOTAL'?'总目标':'每日目标'" required><el-input-number v-model="routineForm.targetValue" :min="0.0001" :precision="4" style="width:100%" /></el-form-item></el-col><el-col :sm="12" :xs="24"><el-form-item label="单位" :required="routineForm.targetMode!=='NONE'"><el-select :model-value="routineForm.unit" @update:model-value="changeRoutineUnit" filterable allow-create default-first-option placeholder="选择或输入单位" style="width:100%"><el-option v-if="routineForm.unit && !commonKpiUnits.includes(routineForm.unit)" :label="routineForm.unit" :value="routineForm.unit"/><el-option v-for="unit in commonKpiUnits" :key="unit" :label="unit" :value="unit" /></el-select></el-form-item></el-col></el-row>
    ...
    <el-dialog v-model="ownerDialog" title="更换项目主负责人" width="min(520px, 94vw)" append-to-body>
      <el-alert title="可选择原负责人继续参与或退出项目；退出时将未完任务及持续工作交接给新负责人，保留交接历史和已计价成本。" type="warning" :closable="false" show-icon />
      <el-form :model="ownerForm" label-width="100px" class="decision-form"><el-form-item label="新负责人" required><el-select v-model="ownerForm.ownerUserId" filterable style="width:100%"><el-option v-for="u in ownerCandidates" :key="u.userId" :label="userOptionLabel(u)" :value="u.userId" /></el-select></el-form-item><el-form-item label="原负责人"><el-radio-group v-model="ownerForm.exitOldOwner"><el-radio :value="false">继续作为成员</el-radio><el-radio :value="true">退出项目，未完工作交接给新负责人</el-radio></el-radio-group></el-form-item><el-form-item label="变更原因" required><el-input v-model="ownerForm.reason" type="textarea" :rows="4" maxlength="500" show-word-limit /></el-form-item></el-form>
    ...
    <el-dialog v-model="acceptanceDialog" title="提交项目验收资料" width="min(660px, 94vw)" append-to-body>
      ...
      <template #footer><el-button @click="acceptanceDialog=false">取消</el-button><el-button type="success" :loading="saving" @click="saveAcceptance">提交{{ acceptanceReviewerLabel }}验收</el-button></template>
    </el-dialog>

    <el-dialog v-model="reviewDialog" :title="reviewForm.decision==='APPROVED'?'验收通过、结项并冻结':'退回项目继续执行'" width="min(560px, 94vw)" append-to-body>
      <el-alert :title="reviewForm.decision==='APPROVED'?'系统将同时确认最终核算、结项并冻结项目数据；付款凭证和受控调账仍可登记。':'退回后项目恢复执行，负责人可修改后再次提交。'" :type="reviewForm.decision==='APPROVED'?'success':'warning'" :closable="false" show-icon />
...
const goalModeLabel={TOTAL:'有项目总目标',NO_TOTAL:'持续经营（不计入公司总目标）'}
const frequencyLabel={DAILY:'每日',WEEKLY:'每周',MONTHLY:'每月'}
const routineTargetModeLabel={FIXED:'固定每日目标',AUTO_TOTAL:'总目标自动分配',DAILY_DYNAMIC:'客户动态日目标',NONE:'无量化目标'}
...
const acceptanceReviewerLabel=computed(()=>detail.value?.governanceProfile?.acceptanceReviewerLabel||(detail.value?.parentId?'主项目主负责人':'归属老板'))
const acceptanceLabel=computed(()=>({PENDING:`待${acceptanceReviewerLabel.value}验收`,APPROVED:`${acceptanceReviewerLabel.value}已通过`,RETURNED:`${acceptanceReviewerLabel.value}已退回`}))
...
const eventLabel = {
  ...
  CLOSE:'项目交付关闭',CLOSE_AND_FREEZE:'结项、核算并冻结',ACCOUNTING_CLOSE:'项目核算关闭',CANCEL:'取消项目',MEMBER_SAVE:'维护项目成员',MEMBER_REMOVE:'移除项目成员',
  TASK_SAVE:'安排/调整一次性任务',TASK_PROGRESS:'更新任务进度',TASK_VOID:'停用一次性任务',TASK_ENABLE:'启用一次性任务',PROJECT_PROGRESS:'填报项目进度',
  ROUTINE_SAVE:'安排/调整持续工作',ROUTINE_VOID:'停用持续工作',ROUTINE_ENABLE:'启用持续工作',ROUTINE_REPORT:'填报持续工作成果',ROUTINE_TARGET_SAVE:'下达/修改今日目标',
...
const commonKpiUnits=computed(()=>['个','件','项','次','单','条','人','户',...(detail.value?.baseCurrency==='CNY'?['元','万元','CNY']:[detail.value?.baseCurrency||'CNY']),'%','分','小时','天'])
...
const governanceDescription=computed(()=>{const mode={LIGHT:'保留任务、成本和KPI，风险按异常处理。',STANDARD:'执行周度跟踪、里程碑、风险台账和预算预警。',KEY_CONTROL:'执行强化里程碑、风险、预算分级预警和治理变更管控。'}[detail.value?.managementMode]||'';const reviewer=acceptanceReviewerLabel.value;const close={DIRECT:`${reviewer}核对前置条件后直接关闭。`,RESULT_ACCEPTANCE:`负责人提交整体验收资料，${reviewer}通过后关闭。`,STAGED_ACCEPTANCE:`负责人逐里程碑提交验收，由${reviewer}审核，全部通过后方可关闭。`}[detail.value?.closeMethod]||'';return `${mode}${close}${detail.value?.acceptanceCriteria?` 验收标准：${detail.value.acceptanceCriteria}`:''}`})
...
const scheduleStatusText=computed(()=>detail.value?.goalMode==='NO_TOTAL'?'不限期项目以每日成果持续跟踪':scheduleProgress.value>projectProgress(detail.value)?`进度落后时间计划 ${scheduleProgress.value-projectProgress(detail.value)} 个百分点`:'当前进度不落后于时间计划')
...
const kpiClosureState=computed(()=>{
  ...
  const meta={
    ...
    DRAFT:cycleNotEnded?{tone:'info',label:'考核中',title:'KPI尚未全部达标，周期仍在进行',description:`${period} 全部指标提前达标可立即确认并办理结项；否则需等周期结束。`,actionLabel:'查看KPI进度',actionType:'primary'}:{tone:'warning',label:'待填报',title:'KPI结果尚未完成结算',description:`${period} 仍在填报中；负责人提交结果后系统立即确认并计入项目核算。`,actionLabel:myRole.value==='OWNER'?'填报并确认KPI':'查看填报进度',actionType:'warning'}
...
const stageClosureState=computed(()=>{
  ...
  if(detail.value?.status==='ACCEPTANCE')return {...base,tone:'warning',label:`待${acceptanceReviewerLabel.value}检验`,title:'负责人已提交项目结项申请',description:canReviewAcceptance.value?'请检验全部阶段成果，确认通过后项目才会结项。':`正在等待${acceptanceReviewerLabel.value}检验，子项目负责人不能自行通过验收。`,canReview:canReviewAcceptance.value}
  ...
  return {...base,tone:'success',label:'可以申请',title:'所有里程碑已完成，结项申请条件已满足',description:myRole.value==='OWNER'?`提交后由${acceptanceReviewerLabel.value}检验，通过后项目才会结项。`:'等待项目负责人提交结项申请。',canRequest:myRole.value==='OWNER'}
})
...
const eventActorName=event=>event?.operatorName||event?.operatorAccount||'系统'
const eventActorAccount=event=>event?.operatorAccount||(eventActorName(event)==='系统'?'SYSTEM':'未记录')
...
const formatEventComment=event=>{
  ...
  if(event?.eventType==='ROUTINE_SAVE'&&slashParts.length>=3){
    ...
    return `持续工作：${name}；目标频率：${frequencyLabel[frequency]||frequency}；${frequency==='DAILY'?'每日目标':'历史周期目标'}：${target.join(' / ')}`
  }
  if(event?.eventType==='TASK_PROGRESS'&&slashParts.length>=2)return `一次性任务：${slashParts[0]}；当前进度：${slashParts.slice(1).join(' / ')}`
  if(event?.eventType==='PROJECT_PROGRESS'&&slashParts.length>=2)return `项目进度：${slashParts[0]}；进展说明：${slashParts.slice(1).join(' / ')}`
  if(event?.eventType==='COST_ALLOCATION'&&slashParts.length>=3)return `成员：${slashParts[0]}；投入方式：${allocationModeLabel[slashParts[1]]||slashParts[1]}；计划投入：${slashParts.slice(2).join(' / ')}${slashParts[1]==='PERCENTAGE'&&!String(slashParts[2]).includes('%')?'%':''}`
  if(event?.eventType==='EFFORT_DAY_CONFIRMED'&&slashParts.length>=3)return `成员：${slashParts[0]}；日期：${slashParts[1]}；确认投入：${slashParts.slice(2).join(' / ')}`
  if(event?.eventType==='EFFORT_DAY_RETURNED'&&slashParts.length>=3)return `成员：${slashParts[0]}；日期：${slashParts[1]}；${slashParts.slice(2).join(' / ')}`
  const fieldLabels={TASK_SAVE:'一次性任务',MILESTONE_SAVE:'里程碑',RISK_SAVE:'风险事项',KPI_CHANGE:'KPI',KPI_RETIRE:'KPI'}
  if(fieldLabels[event?.eventType]&&value)return `${fieldLabels[event.eventType]}：${value}`
  if(event?.eventType==='ROUTINE_REPORT'){
    const separator=value.indexOf('：')
    if(separator>0)return `持续工作：${value.slice(0,separator)}；本次完成：${value.slice(separator+1)}`
  }
  return value
}
...
const eventSummary=event=>{
  ...
  if(['TASK_SAVE','ROUTINE_SAVE'].includes(event?.eventType)&&(event?.subjectName||event?.subjectAccount)){
    ...
    const subjectAccount=event.subjectAccount?`（账号：${event.subjectAccount}）`:'（账号未记录）'
    const workType=event.eventType==='TASK_SAVE'?'一次性任务':'持续工作'
    return `${actor} · 给 ${subject}${subjectAccount} 安排了${workType}`
  }
  if(event?.eventType==='MEMBER_SAVE'){
    const member=memberSaveParts(event)
    return member?`${actor} · 将 ${member.name} 设为${member.role}`:`${actor} · 维护项目成员`
  }
  if(event?.eventType==='MEMBER_REMOVE'){
    const subject=event?.subjectName||event?.subjectAccount
    return subject?`${actor} · 移除项目成员 ${subject}`:`${actor} · 移除一名项目成员`
  }
  return `${actor} · ${eventLabel[event?.eventType]||'执行项目操作'}`
}
...
const availableActions = computed(() => {
  ...
  if (isBoss.value && !d.parentId && d.status === 'ACTIVE' && d.closeMethod === 'DIRECT') actions.push({key:'CLOSE',label:'确认结项并冻结',type:'success'})
  if (myRole.value === 'OWNER' && d.status === 'ACTIVE' && d.closeMethod === 'DIRECT') actions.push({key:'REQUEST_CLOSE',label:'申请直接结项',type:'success'})
  if (canActAsProjectOwner.value && d.status === 'ACTIVE' && d.closeMethod === 'RESULT_ACCEPTANCE') actions.push({key:'SUBMIT_ACCEPTANCE',label:'提交成果验收',type:'success'})
  if ((isBoss.value || myRole.value === 'OWNER') && d.status === 'PAUSED') actions.push({key:'RESUME',label:'恢复执行',type:'primary'})
  if (canReviewAcceptance.value && d.status === 'ACCEPTANCE' && d.closeMethod === 'RESULT_ACCEPTANCE') actions.push({key:'OPEN_ACCEPTANCE',label:'查看验收资料',type:'success'})
...
async function saveAcceptanceReview(){if(reviewForm.decision==='RETURNED'&&!reviewForm.comment?.trim())return ElMessage.warning('请填写退回原因');saving.value=true;try{detail.value=(await reviewBusinessProjectAcceptance(detail.value.projectId,reviewForm)).data;reviewDialog.value=false;activeTab.value='acceptance';ElMessage.success(reviewForm.decision==='APPROVED'?'项目已验收结项，核算已确认并冻结':'项目已退回执行');await load()}finally{saving.value=false}}
...
async function saveStageAcceptance(){if(!stageForm.resultSummary?.trim())return ElMessage.warning('请填写阶段结果');if(!stageForm.deliverables?.trim())return ElMessage.warning('请填写阶段交付成果');saving.value=true;try{await submitBusinessProjectStageAcceptance(detail.value.projectId,stageForm);stageDialog.value=false;activeTab.value='stageAcceptance';await refreshDetail();ElMessage.success(`阶段成果已提交，等待${acceptanceReviewerLabel.value}检验`)}finally{saving.value=false}}
...
async function saveItem(){
  ...
  if(isMember&&!itemForm.value.memberId&&!itemForm.value.companyKey)return ElMessage.warning('请先选择公司')
  if(isMember&&!itemForm.value.userId)return ElMessage.warning('请选择人员')
  if(setsAllocation){
    const percent=itemForm.value.allocationPercent
    if(percent===null||percent===undefined)return ElMessage.warning('请填写成员投入比例')
    if(!Number.isFinite(Number(percent))||Number(percent)<=0||Number(percent)>100)return ElMessage.warning('成员投入比例必须大于0且不超过100%')
  ...
  try{
    ...
    ElMessage.success(setsAllocation?(response.data?.allocationOutcome==='PENDING'?'成员已加入，投入调整已提交相关负责人确认':'成员及投入比例已保存'):'保存成功')
  }finally{saving.value=false}
}
...
const kpiPendingText=item=>item.dataStatus==='PENDING_COST'?'待成本计价':item.automatic||automaticKpiSources.includes(item.sourceType)?'自动统计中':'待填报'
const kpiActualText=item=>kpiHasActual(item)?`${item.actualValue} ${item.unit||''}`:kpiPendingText(item)
const kpiGapText=item=>{if(!kpiHasActual(item))return `距离目标：${kpiPendingText(item)}`;const gap=Number(item.targetValue||0)-Number(item.actualValue||0);if(item.direction==='LOWER_BETTER')return gap>=0?'已达到目标':`超过目标上限 ${money(Math.abs(gap))} ${item.unit||''}`;return gap<=0?'已达到目标':`距离目标还差 ${money(gap)} ${item.unit||''}`}
...
function openKpiDialog(row={}){Object.assign(kpiForm,{kpiId:null,projectId:detail.value.projectId,kpiCode:'',kpiName:'',metricType:'COUNT',targetValue:null,actualValue:null,unit:'',weight:0,direction:'HIGHER_BETTER',aggregateType:'SUM',sourceType:'MANUAL',ownerUserId:null,remark:'',...row});kpiDialog.value=true}
function changeProjectUnit(form,unit){const previous=form.unit;if(previous===unit)return;const cny=detail.value?.baseCurrency==='CNY';const factor=value=>value==='万元'?10000:['元','CNY'].includes(value)?1:null;const from=cny?factor(previous):null,to=cny?factor(unit):null;if(form.targetValue!=null&&Number(form.targetValue)!==0){const converted=from&&to?Number(form.targetValue)*from/to:null;if(converted!=null&&Number.isFinite(converted)&&converted>0&&converted<=99999999999999.99&&Math.abs(converted-Number(converted.toFixed(4)))<1e-9)form.targetValue=Number(converted.toFixed(4));else{form.targetValue=null;ElMessage.info('单位已变化，请按新单位重新填写目标值')}}form.unit=unit}
...
async function saveKpi(){if(!kpiForm.kpiName?.trim())return ElMessage.warning('请填写KPI名称');if(kpiForm.targetValue===null||kpiForm.targetValue===undefined)return ElMessage.warning('请填写目标值');saving.value=true;try{await saveBusinessProjectKpi({...kpiForm,projectId:detail.value.projectId});kpiDialog.value=false;await refreshDetail();ElMessage.success(kpiForm.kpiId?'KPI新版本已生效':'KPI已创建，编码已自动生成')}finally{saving.value=false}}
async function retireKpi(row){await ElMessageBox.confirm(`确定停用“${row.kpiName}”吗？历史版本仍会保留。`,'停用KPI',{type:'warning'});await retireBusinessProjectKpi(detail.value.projectId,row.kpiId);await refreshDetail();ElMessage.success('KPI已停用')}
...
async function saveAllocation(){if(!allocationForm.userId)return ElMessage.warning('请选择项目人员');if(allocationForm.allocationValue===null||allocationForm.allocationValue===undefined)return ElMessage.warning('请填写计划投入');if(Number(allocationForm.allocationValue)>100)return ElMessage.warning('项目投入比例不能超过100%');if(allocationFollowProject.value)syncAllocationProjectPeriod(true);const from=allocationDates.value?.[0]||todayText(),to=allocationDates.value?.[1]||null;saving.value=true;try{await saveBusinessStaffAllocation({...allocationForm,allocationMode:'PERCENTAGE',exceptionAllowed:'0',exceptionReason:'',projectId:detail.value.projectId,effectiveFrom:from,effectiveTo:to});allocationDialog.value=false;await refreshDetail();ElMessage.success('成员计划投入已保存')}finally{saving.value=false}}
async function removeAllocation(row){await ElMessageBox.confirm('确定停用这条人员成本分摊吗？历史核算数据不会删除。','停用分摊',{type:'warning'});await removeBusinessStaffAllocation(detail.value.projectId,row.allocationId);await refreshDetail();ElMessage.success('分摊已停用')}
function openRoutine(row={}){Object.assign(routineForm,{routineId:null,projectId:detail.value.projectId,routineName:'',frequency:'DAILY',targetMode:'FIXED',targetValue:null,unit:'条',assigneeUserId:detail.value.mainOwnerUserId,startDate:detail.value.planStartDate||todayText(),endDate:detail.value.planEndDate||null,remark:'',version:null,...row,targetMode:row.targetMode||'FIXED',frequency:'DAILY',evidenceRequired:'0'});routineLongTerm.value=!!routineForm.startDate&&!routineForm.endDate;routineDialog.value=true}
async function saveRoutine(){if(!routineForm.routineName?.trim())return ElMessage.warning('请填写持续工作内容');if(!routineForm.assigneeUserId)return ElMessage.warning('请选择负责人');if(['FIXED','AUTO_TOTAL'].includes(routineForm.targetMode)&&!(Number(routineForm.targetValue)>0))return ElMessage.warning('目标数量必须大于0');if(routineForm.targetMode!=='NONE'&&!routineForm.unit?.trim())return ElMessage.warning('请填写目标单位');if(!routineForm.startDate)return ElMessage.warning('请选择执行开始日期');if(routineForm.targetMode==='AUTO_TOTAL'&&routineLongTerm.value)return ElMessage.warning('总目标自动分配必须设置结束日期');if(!routineLongTerm.value&&!routineForm.endDate)return ElMessage.warning('请选择执行结束日期或勾选长期');if(routineForm.endDate&&routineForm.startDate>routineForm.endDate)return ElMessage.warning('执行结束日期不能早于开始日期');saving.value=true;try{await saveBusinessRoutine({...routineForm,frequency:'DAILY',targetValue:['FIXED','AUTO_TOTAL'].includes(routineForm.targetMode)?routineForm.targetValue:0,unit:routineForm.unit||'项',projectId:detail.value.projectId,endDate:routineLongTerm.value?null:routineForm.endDate});routineDialog.value=false;activeTab.value='routines';await refreshDetail();ElMessage.success('持续工作已保存')}finally{saving.value=false}}
...
function routineTargetText(row){const mode=row.targetMode||'FIXED';if(mode==='NONE')return '只填每日完成说明';if(mode==='DAILY_DYNAMIC')return row.todayTarget?`今日 ${row.todayTarget} ${row.unit}`:'由负责人每日下达';if(mode==='AUTO_TOTAL')return `总量 ${row.targetValue} ${row.unit}，按剩余天数自动分配`;return `每日 ${row.targetValue} ${row.unit}`}
...
async function enableRoutine(row){
  ...
  if((row.targetMode||'FIXED')==='AUTO_TOTAL'){
    ...
    if(remaining<=0)return ElMessage.warning('该持续工作的总目标已经完成，无需重新启用')
    endDate=row.endDate&&row.endDate>=todayText()?row.endDate:null
    if(!endDate){
      const result=await ElMessageBox.prompt(`剩余目标 ${money(remaining)} ${row.unit||''}。请选择新的执行结束日期。`,'重新启用总目标自动分配',{type:'warning',inputType:'date',inputValue:todayText(),inputValidator:value=>!value?'请选择新的结束日期':value<todayText()?'结束日期不能早于今天':true,confirmButtonText:'确认启用',cancelButtonText:'取消'})
      endDate=result.value
    }else{
      const days=Math.floor((dateMs(endDate)-dayStart())/86400000)+1
      const daily=days>0?remaining/days:remaining
      await ElMessageBox.confirm(`剩余目标 ${money(remaining)} ${row.unit||''}，执行至 ${endDate}，预计每日 ${money(daily)} ${row.unit||''}。确定重新启用吗？`,'启用持续工作',{type:'success'})
    }
  }else await ElMessageBox.confirm(`确定重新启用“${row.routineName}”吗？新的执行区间将从今天开始。`,'启用持续工作',{type:'success'})
  await enableBusinessRoutine(detail.value.projectId,row.routineId,endDate?{endDate}:undefined)
  await refreshDetail()
  ElMessage.success('持续工作已启用，已开启新的执行区间')
}
async function enableTask(row){await ElMessageBox.confirm(`确定重新启用“${row.taskName}”吗？新的执行区间将从今天开始。`,'启用一次性任务',{type:'success'});await enableBusinessTask(detail.value.projectId,row.taskId);await refreshDetail();ElMessage.success('一次性任务已启用，已开启新的执行区间')}
