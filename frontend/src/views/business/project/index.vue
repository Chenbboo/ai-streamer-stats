        ...
        <el-tabs ref="detailTabs" v-model="activeTab" class="project-detail-tabs">
          <el-tab-pane label="项目总览" name="overview">
            ...
            <section class="cockpit-metrics" v-loading="cockpitLoading">
              ...
              <el-alert v-if="cockpit.budgetPeriodExpired" title="本期预算已到期，请在计划基线与变更中续编；此处保留上一期使用情况。" type="warning" :closable="false"/><article v-if="isDailyBudget"><span>每日统计口径</span><b>{{ (operating.budgetScope||detail.budgetScope)==='CASH_EXPENSE'?'仅外部支出':'全成本' }}</b><small>按业务日分别预警；启动预算 {{ money(operating.startupBudgetLimit??detail.startupBudgetLimit) }}</small></article><article v-else :class="budgetTone"><span>{{ detail.budget?'本期预算已使用':cockpitCostIncomplete?'已核算预算使用':'预算已使用' }}</span><b>{{ money(cockpitBudgetSpent) }}</b><small v-if="cockpitCostIncomplete">剩余预算待成本完整后确认</small><small v-else>{{ budgetUsage==null?'—':`${budgetUsage}%` }} · 剩余 {{ money(cockpitBudgetRemaining) }}</small></article>
