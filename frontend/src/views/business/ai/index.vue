          ...
          <div v-for="(message,index) in aiMessages" :key="index" class="ai-message" :class="`is-${message.role}`">
            ...
            <div class="message-stack">
              ...
              <article v-if="message.actionRequest" class="action-card" :class="`is-${message.actionRequest.status?.toLowerCase()}`">
                ...
                <template v-else-if="isCapabilityProjectCreate(message.actionRequest)"><div class="action-grid"><div><small>归属公司</small><b>{{ message.actionRequest.details?.companyName }}</b></div><div><small>主负责人</small><b>{{ message.actionRequest.details?.mainOwnerName }}</b></div><div><small>计划周期</small><b>{{ message.actionRequest.details?.planStartDate }} 至 {{ message.actionRequest.details?.planEndDate }}</b></div><div><small>核算方式</small><b>{{ accountingLabel[message.actionRequest.details?.accountingMode] || message.actionRequest.details?.accountingMode }}</b></div><div><small>管理模式</small><b>{{ managementLabel[message.actionRequest.details?.managementMode] || message.actionRequest.details?.managementMode }}</b></div><div><small>结项方式</small><b>{{ closeMethodLabel[message.actionRequest.details?.closeMethod] || message.actionRequest.details?.closeMethod }}</b></div></div><div class="objective"><small>项目目标</small><p>{{ message.actionRequest.details?.objective }}</p></div><div class="objective"><small>确认后会发生什么</small><p>项目治理方式将随项目资料保存，并保留本次确认和操作审计记录。</p></div></template>
                ...
                <template v-else><div class="action-grid"><div><small>归属公司</small><b>{{ message.actionRequest.project?.companyName }}</b></div><div><small>主负责人</small><b>{{ message.actionRequest.project?.mainOwnerName }}</b></div><div><small>计划周期</small><b>{{ message.actionRequest.project?.planStartDate }} 至 {{ message.actionRequest.project?.planEndDate }}</b></div><div><small>核算方式</small><b>{{ accountingLabel[message.actionRequest.project?.accountingMode] || message.actionRequest.project?.accountingMode }}</b></div><div><small>管理模式</small><b>{{ managementLabel[message.actionRequest.project?.managementMode] || message.actionRequest.project?.managementMode }}</b></div><div><small>结项方式</small><b>{{ closeMethodLabel[message.actionRequest.project?.closeMethod] || message.actionRequest.project?.closeMethod }}</b></div></div><div class="objective"><small>项目目标</small><p>{{ message.actionRequest.project?.objective }}</p></div></template>
...
const managementLabel={LIGHT:'轻量模式',STANDARD:'标准模式',KEY_CONTROL:'重点监管',SIMPLE:'轻量模式',DELIVERY:'标准模式'}
