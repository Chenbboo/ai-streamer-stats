<template>
  <div class="app-container my-page">
    <header class="welcome">
      <div><span>MY WORKSPACE</span><h1>{{ $tr("{0}，今天先处理这些事", [userStore.nickName || userStore.name]) }}</h1><p>{{ $tr("只展示你参与的项目和分配给你的未完成任务。") }}</p></div>
      <el-button icon="Refresh" :loading="loading" @click="load">{{ $tr("刷新") }}</el-button>
    </header>
    <section class="quick-grid">
      <article><b>{{ summary.totalCount || 0 }}</b><span>{{ $tr("参与项目") }}</span></article><article><b>{{ summary.activeCount || 0 }}</b><span>{{ $tr("执行中") }}</span></article><article class="warn"><b>{{ summary.overdueProjectCount || 0 }}</b><span>{{ $tr("存在逾期") }}</span></article><article class="risk"><b>{{ summary.highRiskProjectCount || 0 }}</b><span>{{ $tr("高风险项目") }}</span></article>
    </section>
    <div class="content-grid">
      <section class="panel">
        <div class="panel-head"><div><h2>{{ $tr("我的待办任务") }}</h2><p>{{ $tr("按截止时间优先排序") }}</p></div></div>
        <div v-if="!tasks.length && !loading" class="empty">{{ $tr("当前没有未完成任务") }}</div>
        <button v-for="task in tasks" :key="task.taskId" class="task-row" @click="openProject(task.projectId)">
          <span class="priority" :class="`p-${task.priority?.toLowerCase()}`"></span><span class="task-main"><b>{{ task.taskName }}</b><small>{{ task.projectName }} · {{ task.projectNo }}</small></span><span class="task-state">{{ taskStatusLabel[task.status] || task.status }}<small :class="{ overdue:isOverdue(task.dueDate) }">{{ task.dueDate || $tr("未设期限") }}</small></span>
        </button>
      </section>
      <section class="panel">
        <div class="panel-head"><div><h2>{{ $tr("参与的项目") }}</h2><p>{{ $tr("最近更新的项目") }}</p></div><el-button link type="primary" @click="router.push('/business/projects')">{{ $tr("全部") }}</el-button></div>
        <button v-for="project in projects" :key="project.projectId" class="project-card" @click="openProject(project.projectId)">
          <span class="project-top"><b>{{ project.projectName }}</b><el-tag size="small" :type="statusTone[project.status] || 'info'">{{ projectStatusLabel(project) }}</el-tag></span><small>{{ project.mainOwnerName }} · {{ project.planEndDate ? $tr("计划至 {0}", [project.planEndDate]) : $tr("未设结束日期") }}</small><el-progress :percentage="progress(project)" :stroke-width="6" />
        </button>
        <div v-if="!projects.length && !loading" class="empty">{{ $tr("尚未加入项目") }}</div>
      </section>
    </div>
  </div>
</template>
...
const projectStatusLabel=project=>project?.status==='ACCEPTANCE'&&project?.closeMethod==='STAGED_ACCEPTANCE'?translateText("待结项"):statusLabel[project?.status]||project?.status
const taskStatusLabel={TODO:translateText("待开始"),DOING:translateText("进行中"),BLOCKED:translateText("受阻")}
