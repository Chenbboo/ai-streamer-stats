  ...
  <div class="app-container kpi-page" v-loading="loading">
    <header class="kpi-hero">
      ...
      <div class="hero-tools">
        ...
        <el-button v-if="resultsOnly && selectedProjectId" :disabled="loading" @click="openSettings">返回 KPI 设置</el-button>
    ...
    <template v-if="workspace.project">
      ...
      <div class="content-grid">
        <main>
          ...
          <el-card v-if="resultsOnly" shadow="never" class="section-card">
            <div class="section-head">
              <div><h2>项目结果与结算</h2><p>自动指标持续读取系统数据，手工指标由负责人填报；全部指标提前达标可立即确认，否则在周期结束后确认。</p></div>
              <div class="section-actions" v-if="settlement && workspace.canSettle && ['DRAFT','RETURNED'].includes(settlement.status)">
                ...
                <el-button v-hasPermi="['business:kpi:settle']" type="primary" :loading="saving" :disabled="!canFinishCycle || !canConfirmResults" @click="submitResults">{{ needsLegacyReview ? '重新提交审核' : '确认指标结果' }}</el-button>
              </div>
            </div>
            ...
            <el-alert v-if="settlement && !periodEnded && allTargetsMet && ['DRAFT','RETURNED'].includes(settlement.status)" title="全部KPI已达标，可立即确认结果；确认后今天作为本期实际结束日。" type="success" :closable="false" show-icon />
            <el-alert v-else-if="settlement && !periodEnded && ['DRAFT','RETURNED'].includes(settlement.status)" :title="`KPI尚未全部达标；若不提前达标，可在 ${settlement.periodEnd} 周期结束后确认。`" type="info" :closable="false" show-icon />
              ...
              <article v-for="item in selectedPlan.items || []" :key="item.itemId" class="result-row">
                ...
                <template v-if="workspace.canSettle && ['DRAFT','RETURNED'].includes(settlement?.status) && !isAutomatic(item)">
                  <div class="manual-result-input">
                    <el-input-number v-model="resultDraft[item.itemId].actualValue" :min="0" :precision="manualAmountInYuan(item)?2:4" controls-position="right" :placeholder="manualAmountInYuan(item)?'实际金额（元）':'实际值'" />
                    <small v-if="manualAmountInYuan(item)">输入元；折合 {{ actualFor(item) ?? '—' }} 万元</small>
                    <small v-else-if="item.unit">输入单位：{{ item.unit }}</small>
                  </div>
                ...
                <template v-else>
                  ...
                  <el-button v-if="settlement?.status==='CONFIRMED' && !isLegacyPlan && workspace.canSettle && item.sourceType==='MANUAL'" v-hasPermi="['business:kpi:settle']" link type="primary" @click="openCorrection(item)">更正已确认结果</el-button>
        ...
        <aside>
          <el-card v-if="!resultsOnly" shadow="never" class="section-card result-entry">
            <div class="section-head"><div><h2>KPI 结果填报</h2><p>考核完成后，在这里填写实际完成值和结果说明，再确认本期结果。</p></div></div>
            <template v-if="selectedPlan">
              <p class="result-entry-period">当前选择：v{{ selectedPlan.planVersion }} · {{ selectedPlan.cycleStart }} 至 {{ selectedPlan.cycleEnd }}</p>
              <el-button v-hasPermi="['business:kpi:list']" type="primary" :disabled="loading" @click="openResults">{{ workspace.canSettle && ['DRAFT','RETURNED'].includes(settlement?.status) && selectedPlan.status!=='VOIDED' ? '填报 KPI 结果' : '查看 KPI 结果' }}</el-button>
              <small>下方选择其他历史方案，可查看或办理对应期的结果。</small>
            </template>
            <el-empty v-else :image-size="45" description="先发布考核方案，再填报对应结果" />
          </el-card>
          <el-card v-if="!resultsOnly" shadow="never" class="section-card">
    ...
    <el-dialog v-model="targetDialog" :title="targetForm.kpiId ? '调整项目KPI目标' : '新增项目KPI'" width="min(680px,94vw)" append-to-body>
      ...
      <el-form :model="targetForm" label-width="112px" class="dialog-form">
        ...
        <el-row :gutter="12"><el-col :sm="12" :xs="24"><el-form-item label="目标值" required><el-input-number v-model="targetForm.targetValue" :min="0.0001" :precision="4" style="width:100%" /></el-form-item></el-col><el-col :sm="12" :xs="24"><el-form-item label="单位"><el-select :model-value="targetForm.unit" @update:model-value="changeTargetUnit" filterable allow-create default-first-option clearable placeholder="选择或输入单位" style="width:100%"><el-option v-for="unit in commonKpiUnits" :key="unit" :label="unit" :value="unit" /></el-select><small v-if="targetForm.unit==='万元'" class="field-help">目标按万元设置；手工结果输入元，系统会换算为万元。</small></el-form-item></el-col></el-row>
    ...
    <el-dialog v-model="planDialog" title="发布项目指标方案" width="min(900px,96vw)" append-to-body>
      ...
      <el-form :model="planForm" label-width="92px" class="dialog-form">
        <el-row :gutter="12">
          <el-col :sm="10" :xs="24"><el-form-item label="考核周期" required><el-select v-model="planForm.cycleType" style="width:100%" @change="resetPlanDates"><el-option label="月度" value="MONTH"/><el-option label="季度" value="QUARTER"/><el-option label="项目周期（需要结束日期）" value="PROJECT" :disabled="!projectPeriodReady"/></el-select><small v-if="!workspace.project?.planEndDate" class="field-help">不限期项目请按月度或季度发布 KPI 方案。</small></el-form-item></el-col>
          <el-col :sm="14" :xs="24">
    ...
    <el-dialog v-model="correctionDialog" title="更正已确认 KPI 结果" width="min(520px,94vw)" append-to-body>
      <el-alert title="更正会重算本期综合得分并记录原因；已确认的考核结束日期不会自动改变。" type="warning" :closable="false" show-icon />
      <el-form label-width="96px" class="dialog-form">
        <el-form-item label="指标">{{ correctionForm.item?.kpiName }}</el-form-item>
        <el-form-item label="当前结果">{{ correctionForm.original }} {{ correctionForm.item?.unit || '' }}</el-form-item>
        <el-form-item :label="manualAmountInYuan(correctionForm.item)?'正确金额（元）':'正确实际值'" required>
          ...
          <small v-if="manualAmountInYuan(correctionForm.item)" class="field-help">折合 {{ correctionForm.actualInput==null?'—':Number((Number(correctionForm.actualInput)/10000).toFixed(8)) }} 万元</small>
        </el-form-item>
        <el-form-item label="更正原因" required><el-input v-model="correctionForm.reason" type="textarea" :rows="3" maxlength="500" show-word-limit placeholder="说明原数据为什么有误" /></el-form-item>
      </el-form>
      <template #footer><el-button @click="correctionDialog=false">取消</el-button><el-button type="primary" :loading="saving" @click="submitCorrection">确认更正</el-button></template>
    </el-dialog>
  </div>
</template>
...
const commonKpiUnits=computed(()=>['个','件','项','次','单','条','人','户',...(workspace.project?.baseCurrency==='CNY'?['元','万元','CNY']:[workspace.project?.baseCurrency||'CNY']),'%','分','小时','天'])
...
function changeSourceType(sourceType){targetForm.sourceRefId=null;if(['REVENUE','BUSINESS_COST','PERSONNEL_COST','PROFIT'].includes(sourceType)){targetForm.metricType='AMOUNT';const currency=workspace.project?.baseCurrency||'CNY';if(targetForm.unit!==currency&&!(currency==='CNY'&&['元','万元'].includes(targetForm.unit)))changeTargetUnit(currency)}else if(['TASK','MILESTONE'].includes(sourceType)){targetForm.metricType='COUNT';changeTargetUnit('项')}}
...
const proposalTargetOptions=computed(()=>{
 ...
 if(Number(workspace.proposalEstimatedRevenue)>0)items.unshift({key:'revenue',label:'预计收入',revenue:true})
 return items
})
...
async function submitCorrection(){const item=correctionForm.item;if(!item||correctionForm.actualInput==null)return ElMessage.warning('请填写正确实际值');if(!correctionForm.reason.trim())return ElMessage.warning('请填写更正原因');const actualValue=manualAmountInYuan(item)?Number((Number(correctionForm.actualInput)/10000).toFixed(8)):correctionForm.actualInput;await ElMessageBox.confirm(`将“${item.kpiName}”更正为 ${actualValue} ${item.unit||''}，并重算本期综合得分。确定继续吗？`,'确认更正',{type:'warning'});saving.value=true;try{await correctProjectKpiResult(settlement.value.settlementId,{planItemId:item.itemId,actualValue,resultNote:correctionForm.reason.trim()});correctionDialog.value=false;await loadWorkspace(selectedProjectId.value,selectedPlan.value.planId);ElMessage.success('KPI结果与综合得分已更正，原因已记录')}finally{saving.value=false}}
