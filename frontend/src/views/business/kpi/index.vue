      ...
      <div class="content-grid">
        ...
        <aside>
          ...
          <el-card shadow="never" class="section-card">
            ...
            <button v-for="plan in workspace.plans || []" :key="plan.planId" type="button" class="plan-row" :class="{active:Number(plan.planId)===Number(selectedPlan?.planId)}" @click="loadWorkspace(selectedProjectId,plan.planId)">
              <span><b>v{{ plan.planVersion }} · {{ cycleLabel[plan.cycleType] }}</b><small>{{ plan.cycleStart }} 至 {{ plan.cycleEnd }}</small></span>
              <el-tag :type="settlementTone[plan.settlementStatus]">{{ settlementLabel[plan.settlementStatus] }}</el-tag>
            </button>
          </el-card>
        </aside>
      </div>
...
import { getProjectKpiWorkspace, publishProjectKpiPlan, reviewProjectKpiSettlement, saveProjectKpiResults, submitProjectKpiSettlement } from '@/api/business/kpi'

const route=useRoute(),router=useRouter()
const loading=ref(false),saving=ref(false),projects=ref([]),selectedProjectId=ref(null),workspace=reactive({}),targetDialog=ref(false),targetForm=reactive({}),planDialog=ref(false),planForm=reactive({}),planDates=ref([]),resultDraft=reactive({})
...
async function loadProjects(){const res=await listBusinessProjects({pageNum:1,pageSize:200,status:''});projects.value=res.rows||[];const requested=route.query.projectId?Number(route.query.projectId):null;selectedProjectId.value=projects.value.some(item=>Number(item.projectId)===requested)?requested:(projects.value[0]?.projectId||null)}
async function loadWorkspace(projectId,planId){if(!projectId)return;loading.value=true;try{const res=await getProjectKpiWorkspace(projectId,planId);Object.keys(workspace).forEach(key=>delete workspace[key]);Object.assign(workspace,res.data||{});selectedProjectId.value=projectId;hydrateDraft();router.replace({query:{projectId,planId:workspace.selectedPlan?.planId||undefined}})}finally{loading.value=false}}
...
onMounted(async()=>{loading.value=true;try{await loadProjects();if(selectedProjectId.value)await loadWorkspace(selectedProjectId.value,route.query.planId?Number(route.query.planId):undefined)}finally{loading.value=false}})
</script>
