...
function actualFor(item){return isAutomatic(item)?resultFor(item.itemId)?.actualValue:resultDraft[item.itemId]?.actualValue}
const allTargetsMet=computed(()=>{const items=selectedPlan.value?.items||[];return !!items.length&&items.every(item=>{const actual=Number(actualFor(item)),target=Number(item.targetValue);if(!Number.isFinite(actual)||!Number.isFinite(target))return false;return item.direction==='LOWER_BETTER'?actual<=target:actual>=target})})
