  ...
  <div class="container">
      ...
      <el-scrollbar class="left-scrollbar">
        <div class="components-list">
          <div class="components-title">
            <svg-icon icon-class="component" />{{ $tr("输入型组件 ") }}</div>
          ...
          <div class="components-title">
            <svg-icon icon-class="component" />{{ $tr("选择型组件 ") }}</div>
          ...
          <div class="components-title">
            <svg-icon icon-class="component" />{{ $tr(" 布局型组件 ") }}</div>
    ...
    <div class="center-board">
      <div class="action-bar">
        <el-button icon="Download" type="primary" text @click="download">{{ $tr(" 导出vue文件 ") }}</el-button>
        <el-button class="copy-btn-main" icon="DocumentCopy" type="primary" text @click="copy">{{ $tr(" 复制代码 ") }}</el-button>
        <el-button class="delete-btn" icon="Delete" text @click="empty" type="danger">{{ $tr(" 清空 ") }}</el-button>
      </div>
      <el-scrollbar class="center-scrollbar">
        <el-row class="center-board-row" :gutter="formConf.gutter">
          <el-form :size="formConf.size" :label-position="formConf.labelPosition" :disabled="formConf.disabled"
            ...
            <div v-show="!drawingList.length" class="empty-info">{{ $tr(" 从左侧拖入或点选组件进行表单设计 ") }}</div>
          </el-form>
        </el-row>
      </el-scrollbar>
    </div>
    ...
    <code-type-dialog v-model="dialogVisible" :title="$tr(&quot;选择生成类型&quot;)" :showFileName="showFileName" @confirm="generate" />
...
function empty() {
  proxy.$modal.confirm(translateText("确定要清空所有组件吗？"), translateText("提示"), { type: 'warning' }).then(() => {
      idGlobal.value = 100
      drawingList.value = []
...
onMounted(() => {
  clipboard = new ClipboardJS('#copyNode', {
    text: trigger => {
      const codeStr = generateCode()
      ElNotification({ title: translateText("成功"), message: translateText("代码已复制到剪切板，可粘贴。"), type: 'success' })
      return codeStr
    }
  })
  clipboard.on('error', e => {
    proxy.$modal.msgError(translateText("代码复制失败"))
  })
})
onUnmounted(() => {
  clipboard.destroy()
