<template>
  <el-drawer v-model="showSettings" :withHeader="false" :lock-scroll="false" direction="rtl" size="300px">
    <div class="setting-drawer-title">
      <h3 class="drawer-title">{{ $tr("菜单导航设置") }}</h3>
    </div>
    <div class="nav-wrap">
      <el-tooltip :content="$tr(&quot;左侧菜单&quot;)" placement="bottom">
        <div class="item left" @click="handleNavType(1)" :class="{ activeItem: navType == 1 }">
          <b></b><b></b>
        </div>
      </el-tooltip>

      <el-tooltip :content="$tr(&quot;混合菜单&quot;)" placement="bottom">
        <div class="item mix" @click="handleNavType(2)" :class="{ activeItem: navType == 2 }">
          <b></b><b></b>
        </div>
      </el-tooltip>
      <el-tooltip :content="$tr(&quot;顶部菜单&quot;)" placement="bottom">
    ...
    <div class="setting-drawer-title">
      <h3 class="drawer-title">{{ $tr("主题风格设置") }}</h3>
    </div>
    <div class="setting-drawer-block-checbox">
      <div class="setting-drawer-block-checbox-item" @click="handleTheme('theme-dark')">
        <img src="@/assets/images/dark.svg" alt="dark" />
        <div v-if="sideTheme === 'theme-dark'" class="setting-drawer-block-checbox-selectIcon" style="display: block;">
          <i :aria-label="$tr(&quot;图标: check&quot;)" class="anticon anticon-check">
    ...
    <h3 class="drawer-title">{{ $tr("系统布局配置") }}</h3>
    ...
    <div class="drawer-item">
      ...
      <span class="comp-style">
        <el-radio-group v-model="settingsStore.tagsViewStyle" :disabled="!settingsStore.tagsView" size="small">
          <el-radio-button label="card">{{ $tr("卡片") }}</el-radio-button>
          <el-radio-button label="chrome">{{ $tr("谷歌") }}</el-radio-button>
        </el-radio-group>
      </span>
    </div>

    <div class="drawer-item">
      <span>{{ $tr("固定 Header") }}</span>
      <span class="comp-style">
    ...
    <el-button type="primary" plain icon="DocumentAdd" @click="saveSetting">{{ $tr("保存配置") }}</el-button>
    <el-button plain icon="Refresh" @click="resetSetting">{{ $tr("重置配置") }}</el-button>
  </el-drawer>
...
function saveSetting() {
  proxy.$modal.loading(translateText("正在保存到本地，请稍候..."))
...
function resetSetting() {
  proxy.$cache.local.remove('tags-view-visited')
  proxy.$modal.loading(translateText("正在清除设置缓存并刷新，请稍候..."))
