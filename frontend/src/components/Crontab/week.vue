<template>
    <el-form>
        <el-form-item>
            <el-radio v-model='radioValue' :value="1">{{ $tr(" 周，允许的通配符[, - * ? / L #] ") }}</el-radio>
        </el-form-item>
        ...
        <el-form-item>
            <el-radio v-model='radioValue' :value="3">{{ $tr(" 周期从 ") }}<el-select clearable v-model="cycle01">
                    <el-option
        ...
        <el-form-item>
            <el-radio v-model='radioValue' :value="4">{{ $tr(" 第 ") }}<el-input-number v-model='average01' :min="1" :max="4" />{{ $tr(" 周的 ") }}<el-select clearable v-model="average02">
        ...
        <el-form-item>
            <el-radio v-model='radioValue' :value="5">{{ $tr(" 本月最后一个 ") }}<el-select clearable v-model="weekday">
                    <el-option v-for="item in weekList" :key="item.key" :label="item.value" :value="item.key" />
                </el-select>
            </el-radio>
        </el-form-item>
        ...
        <el-form-item>
            <el-radio v-model='radioValue' :value="6">{{ $tr(" 指定 ") }}<el-select class="multiselect" clearable v-model="checkboxList" :placeholder="$tr(&quot;可多选&quot;)" multiple :multiple-limit="6">
                    <el-option v-for="item in weekList" :key="item.key" :label="item.value" :value="item.key" />
                </el-select>
            </el-radio>
        </el-form-item>
...
const weekList = ref([
    {key: 1, value: translateText("星期日")},
    {key: 2, value: translateText("星期一")},
    {key: 3, value: translateText("星期二")},
    {key: 4, value: translateText("星期三")},
    {key: 5, value: translateText("星期四")},
    {key: 6, value: translateText("星期五")},
    {key: 7, value: translateText("星期六")}
])
