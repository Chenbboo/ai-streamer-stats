<template>
  <el-dialog v-model="visible" top="5vh" @closed="$emit('closed')" :title="`${data.projectName || '子项目'} · ${mode === 'submit' ? '汇报进度' : '进度与汇报历史'}`" :width="mode === 'submit' ? 'min(860px, 96vw)' : 'min(1120px, 96vw)'" append-to-body :close-on-click-modal="!saving" :close-on-press-escape="!saving" :show-close="!saving">
      ...
      <template v-else-if="!loading">
        ...
        <template v-else>
          <section class="progress-overview">
            ...
            <div class="overview-tip"><b>汇报内容与凭证一起查看</b><p>最新汇报默认展开，历史记录按提交时间保留。</p></div>
          </section>
          <el-button v-if="data.parentId && canSubmit" type="primary" @click="startNew">汇报进度 / 提交纠正版本</el-button>
          ...
          <template v-if="data.parentId || data.reports?.length"><ReportHistory :reports="data.reports" :selected-report-id="selectedReportId" /></template>
...
function startNew(){if(!canSubmit.value)return;Object.assign(form,{progress:data.value.progressPercent??0,completionSummary:'',issuesRisks:'',nextPlan:'',syncTasks:true,syncRoutines:true});mode.value='submit';formRef.value?.clearValidate()}
...
async function submit(){if(!canSubmit.value || saving.value || !formRef.value)return;saving.value=true;try{if(!await formRef.value.validate().catch(()=>false))return;const result=await submitBusinessProjectProgressReport({...form,projectId});mode.value='history';selectedReportId.value=result.data.reportId;ElMessage.success('汇报已提交，历史版本已保留');emit('submitted',{projectId,parentId:data.value.parentId});await load()}finally{saving.value=false}}
defineExpose({open})
</script>
<style scoped>.progress-content{min-height:220px;max-height:calc(90vh - 135px);overflow-y:auto;overflow-x:hidden;padding-right:6px}.report-form{margin-top:18px}.hint{color:#7e8c9d;font-size:12px;margin-left:10px}.aggregate{margin-top:18px}h3{font-size:15px;margin-top:22px}
