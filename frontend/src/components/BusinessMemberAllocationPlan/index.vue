<template>
  <section class="member-allocation-plan" v-loading="loading">
    <div class="allocation-heading">
      <div><b>{{ preview.userName || '所选人员' }}在本项目周期内的项目投入</b>
        <small v-if="ready">周期：{{ preview.periodStartDate }} 至 {{ preview.periodEndDate || '不限期' }}；调整生效日：{{ preview.effectiveDate }}</small>
      </div>
      <div class="allocation-actions"><el-button v-if="userId" size="small" link :disabled="saving || loading" @click="load">重新加载</el-button><el-button size="small" type="primary" :loading="saving" :disabled="!ready || loading || preview.hasPendingRequest" @click="$emit('save')">保存</el-button></div>
    </div>
    <p v-if="!userId" class="allocation-hint">选择人员后，显示周期内参与的项目及投入比例。</p>
    <div v-else-if="error" class="allocation-error"><span>{{ error }}</span><el-button link type="primary" @click="load">重新加载</el-button></div>
    <template v-else-if="ready">
      <p class="allocation-hint">周期内共 {{ periodProjectCount }} 个项目（含本项目）；当前另有 {{ allocations.length }} 个项目可调整。</p>
      <el-alert v-if="preview.hasPendingRequest" type="warning" :closable="false" show-icon title="该人员已有待确认的投入调整，请先处理后重新加载。" />
      <el-table :data="tableRows" size="small" :row-class-name="({row})=>row.current?'current-project':''">
        <el-table-column label="项目 / 周期" min-width="220">
          <template #default="{row}"><b>{{ row.projectName }}</b><small>{{ row.current ? '本次添加成员' : `${row.projectNo || ''} · ${row.editable ? '当前可调整' : periodLabel(row)}` }}</small>
            <small v-if="!row.current && !row.editable">{{ row.projectStartDate || row.joinedDate || '未记录' }} 至 {{ row.projectEndDate || row.leftDate || '不限期' }}</small>
          </template>
        </el-table-column>
        <el-table-column prop="ownerName" label="负责人" min-width="100" />
        <el-table-column label="投入比例" width="180" align="right">
          <template #default="{row}">
            <div v-if="row.current" class="allocation-percent"><el-input-number :model-value="modelValue" @update:model-value="$emit('update:modelValue',$event)" :min="0.01" :max="100" :precision="2" :step="5" controls-position="right" :disabled="saving || preview.hasPendingRequest" aria-label="本项目投入比例"/><span>%</span></div>
            <div v-else-if="row.editable" class="allocation-percent"><el-input-number v-model="row.allocationValue" :min="0" :max="100" :precision="2" :step="5" controls-position="right" :disabled="saving || preview.hasPendingRequest" :aria-label="`${row.projectName}投入比例`"/><span>%</span></div>
            <span v-else>{{ formatPercent(row.allocationValue) }}%</span>
          </template>
        </el-table-column>
      </el-table>
      <div class="allocation-total" :class="{invalid:!validTotal}"><span>当前生效项目合计</span><b>{{ formatPercent(total) }}%</b><small>{{ allocations.length ? '合计必须等于100%' : '本项目投入比例须大于0且不超过100%' }}</small></div>
...
const periodLabel=row=>({ENDED:'已结束项目',UPCOMING:'后续项目',PERIOD:'周期内项目'})[row.periodState]||'周期内项目'
...
async function load(){
  ...
  try{
    ...
    ready.value=true
  }catch(err){if(request===sequence)error.value=err?.message||'项目投入加载失败，请重试'}
  finally{if(request===sequence)loading.value=false}
}
function validate(){
  let message=''
  if(loading.value)message='正在加载项目投入，请稍后保存'
  else if(!ready.value||error.value)message='请先加载该成员的项目投入信息'
  else if(preview.value.hasPendingRequest)message='该人员已有待确认的投入调整，请先处理'
  else if(allocations.value.some(row=>row.allocationValue==null||!Number.isFinite(Number(row.allocationValue))||Number(row.allocationValue)<0||Number(row.allocationValue)>100))message='各项目投入比例须在0%至100%之间'
  else if(!validTotal.value)message=allocations.value.length?'所有当前生效项目投入比例合计必须等于100%':'本项目投入比例须大于0且不超过100%'
  else if(allocations.value.length&&!reason.value.trim())message='请填写跨项目投入调整原因'
