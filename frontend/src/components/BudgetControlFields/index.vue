<template>
  <div class="budget-control-fields">
    ...
    <template v-if="modelValue.mode === 'DAILY'">
      ...
      <details v-if="compact" class="budget-help"><summary>每日预算计算说明</summary><small>逐日检查上限；每周支出按金额÷7、每月按金额÷30摊销，人员按工作日计入。一次性支出单独检查启动预算。</small></details><small v-else>逐日检查上限；每周支出按金额÷7、每月按金额÷30摊销，人员按工作日计入。一次性支出单独检查启动预算。</small>
    </template>
    <el-form-item v-if="modelValue.mode === 'DAILY'" label="一次性启动预算" :prop="fieldProp('startupLimit')" :required="startupRequired"><el-input-number :model-value="modelValue.startupLimit" :min="0" :max="99999999999999.99" :precision="2" aria-label="一次性启动预算" @update:model-value="set('startupLimit',$event)"/><small>{{ compact ? '须覆盖本期一次性支出' : '本期有一次性支出时必填，额度需覆盖本期一次性支出。' }}</small></el-form-item>
