<template>
  <el-button v-if="canManage" @click="open">{{ $tr("设置税率") }}</el-button>
  <el-dialog v-model="visible" :title="$tr(&quot;公司税率设置&quot;)" width="min(620px,94vw)" append-to-body>
    <el-alert :title="$tr(&quot;扣完业务成本、人员成本、奖金、公共费用和管理费后，仅对正的剩余利润计税。&quot;)" type="info" :closable="false" show-icon />
    <el-alert v-if="loadFailed" :title="$tr(&quot;税率读取失败，请重新打开后重试。&quot;)" type="error" :closable="false" show-icon />
    <el-form v-loading="loading" label-width="95px" style="margin-top:20px">
      <el-form-item :label="$tr(&quot;公司&quot;)" required><el-select v-model="companyId" style="width:100%" @change="selectCompany"><el-option v-for="company in companies" :key="company.companyDeptId" :label="company.companyName" :value="company.companyDeptId" /></el-select></el-form-item>
      <el-form-item :label="$tr(&quot;税率&quot;)" required><el-input-number v-model="rate" :min="0" :max="100" :precision="4" :step="1" /> <span style="margin-left:8px">%</span></el-form-item>
      <el-form-item :label="$tr(&quot;设置说明&quot;)" required><el-input v-model="reason" type="textarea" :rows="3" maxlength="500" show-word-limit /></el-form-item>
    </el-form>
    <p class="tax-note">{{ $tr("税后盈利＝扣完全部费用后的利润－税额。新税率用于未结算项目；已结算项目保留当时的税率。未设置时暂按0%，0%也可以明确保存。") }}</p>
    <el-empty v-if="!loading&&!companies.length" :description="$tr(&quot;暂无可管理的公司&quot;)" :image-size="60" />
    <template #footer><el-button @click="visible=false">{{ $tr("取消") }}</el-button><el-button type="primary" :loading="saving" :disabled="loading||loadFailed||!companyId||rate==null||!reason.trim()" @click="save">{{ $tr("保存税率") }}</el-button></template>
  </el-dialog>
</template>
<script setup>
